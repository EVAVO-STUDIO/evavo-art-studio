...
- an OpenAI image adapter using ordered references and masks, explicit high input fidelity for every edit/reference request, model-aware native-alpha requests for supported GPT Image 1.x models, and declared chroma-key fallback for GPT Image 2;
...
- deterministic professional mastering with arbitrary rotation, affine and perspective transforms, exact source/mask rectangle copy-paste, curves, channel mixing, tonal adjustment, blur and edge filters, alpha feathering, defringing, shadows, glows, directional rim light, draft 2D normal maps and release-profile evidence;
- governed timestamp-selected video reference-frame extraction through fixed no-shell FFmpeg/ffprobe execution with exact source, probe, binary and output hashes;
...
- motion-family and identity-locked sequence review measuring alpha bounds, mass, visible colour and centroid-aligned silhouette continuity, with governed smoothstep crossfade previews for video-like review of sparse authored poses;
...
Granular deterministic image surgery is available through `alpha-clean`, `chroma-to-alpha`, `component-prune`, `rect-clear`, `rect-fill` and `clone-stamp`. Together these operations provide canonical true-alpha mastering, bounded effect-safe chroma removal, detached-fragment cleanup, exact selection deletion/fill and local patch copying without mutating the source image.
...
The Project Art sandbox includes the deterministic operation families needed for professional finishing: crop, pad, translate, pixel-safe and continuous-tone resize, arbitrary rotation, affine and perspective transforms, exact source/mask rectangle copy-paste, colour replacement, grayscale, inversion, posterisation, thresholding, gamma, hue, curves, channel mixing, multiple blur and edge filters, alpha morphology, feathering, defringing, halo and matte cleanup, ordered compositing, shadows, glows, directional rim light and bounded normal-map derivation. It can extract exact timestamp-selected video reference frames while fingerprinting FFmpeg/ffprobe; those frames remain unapproved references. Sprite slicing and assembly default to strict decoded transparency admission and retain evidence for every source and output. `image-master` applies a release profile and emits a self-hashed mastering report. `motion-sequence` renders bounded keyframed layers to PNG frames, an exact manifest and an optional GIF preview, while sequence profiles catch identity and silhouette drift. Sequence evidence can also render a capped 30 fps-style smoothstep crossfade preview, including an explicit final-to-first transition for loops, without changing or pretending to approve the real-alpha authored masters.

The avatar animation suite compiler expands pinned EVA or Top Hat identity references into a governed 25-clip production plan: four anti-repeating idles, six talk performances, speech transitions, reactions and a character-specific greeting. It schedules 732 continuity-linked full-character frames at authored 24–30 fps cadence plus 17 pixel-registered mouth and eye layers. Top Hat v2 handoffs retain the canonical portrait while binding every image job to a full-body RGBA master by repository commit, tree, asset hash, manifest hash and reference-set digest. Each frame requires genuine alpha or a declared low-collision chroma matte, border-connected background recovery, colour decontamination and independent frame assurance; a painted checkerboard is always blocking. The compiler plans work only and cannot approve, publish or activate art.

See [`docs/PERSISTENT_ARTIST_WORKSPACE.md`](./docs/PERSISTENT_ARTIST_WORKSPACE.md), [`docs/PROJECT_ART_MASTERING_AND_MOTION.md`](./docs/PROJECT_ART_MASTERING_AND_MOTION.md), [`docs/ARTIST_AUTOMATION_CAPABILITY_MAP.md`](./docs/ARTIST_AUTOMATION_CAPABILITY_MAP.md) and [`docs/PROJECT_ART_AVATAR_ANIMATION_SUITE.md`](./docs/PROJECT_ART_AVATAR_ANIMATION_SUITE.md).

...
5. Start a compatible worker separately only after provider execution is deliberately authorised.
6. Inspect immutable provider evidence and unapproved candidate artifacts.
7. Master, evaluate, approve, bind, and publish through their independent governed boundaries.
...
The create-only `evavo.raw-art-provider-runtime-admission-receipt.v1` receipt records:
...
- each immutable runtime job ID, normalized specification SHA-256, provider request identity, and creation timestamp;
...
Do not start `dev:worker`, `worker:once`, `worker:until-idle`, or any pg-boss delivery process as part of admission. Those are separate execution effects and must remain independently controlled.

...
Before these blocking checks, the background-recovery kernel classifies the source. It preserves meaningful native alpha while canonicalizing hidden RGB; reconstructs confidently detected neutral, chromatic, subtle, nonstandard-tile or resampled painted checkerboards into real alpha; rejects transparent-rim bypasses around either grids or proven solid high-chroma mattes; removes a declared high-chroma matte; or conservatively infers a flat high-chroma border-band matte when a provider deviates. Chroma recovery follows the nearest confident local matte, uses scale-aware inset subject references, enforces physically bounded foreground colours, removes dark matte spill and rejects provider-painted complementary edge colours. The complement detector compares outer-edge chroma against the inset subject along the axis opposite the local matte, catching subtle fringes without treating luminance changes as halo colour. It also repairs visible matte-connected antialias samples while preserving their bounded coverage, preventing colour cleanup from eroding the silhouette. Ordinary pixels retain bounded recomposition proof; distance-triggered, complement-triggered, foreground-edge and connected-matte-edge repairs record their counts and maximum source drift separately. Grid and matte-rim repairs are accepted only when canvas-edge segmentation and recomposition proof succeed; ambiguous backgrounds remain rejected.

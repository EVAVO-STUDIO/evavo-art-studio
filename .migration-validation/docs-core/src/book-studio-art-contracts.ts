...
function isNonEmptyStringArray(value: unknown): value is string[] {
  return Array.isArray(value) && value.length > 0 && value.length <= 256 && value.every((entry) => typeof entry === "string" && entry.trim() === entry && entry.length > 0 && entry.length <= 2_000);
}

function isStringArray(value: unknown): value is string[] {
  return Array.isArray(value) && value.length <= 256 && value.every((entry) => typeof entry === "string" && entry.trim() === entry && entry.length > 0 && entry.length <= 2_000);
}
...
export function validateBookArtworkUseBinding(

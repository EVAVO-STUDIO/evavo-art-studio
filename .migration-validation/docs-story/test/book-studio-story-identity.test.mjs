...
function minimal(overrides = {}) {
  return {
    ...
    volumes: [{
      volumeId: "book-1",
      sequence: 1,
      manuscriptRevisionId: "revision:book-1:1",
      manuscriptSha256: sha("b"),
      canonicalUnitIds: ["unit:book-1:1"],
      startSequence: 0,
      endSequence: 1,
      status: "canonical",
    }],
    ...
    actors: [{
      ...
      kind: "character",
      name: "One actor",
      currentLocationId: "location:one",
      availableFromSequence: 0,
      publicGoalIds: ["goal:one"],
      privateGoalIds: [],
      fearIds: [],
      obligationIds: [],
      secretIds: [],
      resourceIds: [],
      relationshipStateIds: [],
      injuryOrFatigueIds: [],
      activePlanIds: ["plan:one"],
      blockedPlanIds: [],
      nextLikelyActionIds: ["action:one"],
      historicalConstraintIds: [],
      evidenceIds: ["evidence:actor"],
    }],
    knowledge: [],
    plans: [{
      planId: "plan:one",
      ...
      objective: "Act",
      currentStep: "Begin",
      requiredLocationIds: ["location:one"],
      requiredResourceIds: [],
      dependencyPlanIds: [],
      oppositionActorIds: [],
      successConsequenceIds: ["consequence:success"],
      failureConsequenceIds: ["consequence:failure"],
    ...
    events: [{
      ...
      startSequence: 0,
      endSequence: 1,
      locationIds: ["location:one"],
      participantActorIds: ["actor:one"],
      causalEventIds: [],
      enablingPlanIds: ["plan:one"],
      historicalEventIds: [],
      state: "completed",
      publicOutcome: "Done",
      hiddenOutcome: "",
      consequenceIds: ["consequence:success"],
    ...
    canon: [{
      canonId: "canon:one",
      kind: "fact",
      subjectIds: ["subject:one"],
      value: "One fact",
      establishedVolumeId: "book-1",
      establishedUnitId: "unit:book-1:1",
      establishedSequence: 1,
      sourceEvidenceIds: ["evidence:canon"],
      mutable: false,
      status: "active",
    }],
    arcs: [],
    setupsAndPayoffs: [],
    unresolvedActorLocationIds: [],
    unresolvedKnowledgeLeakIds: [],
    unresolvedTimelineConflictIds: [],
    unresolvedTravelConflictIds: [],
    unresolvedMotivationGapIds: [],
    unresolvedCoincidenceIds: [],
    unresolvedOffPageEventIds: [],
    continuityConflictIds: [],
    forgottenConsequenceIds: [],
    repeatedArcIds: [],
    genericSeriesPatternIds: [],
    requiredIndependentReviewIds: ["review:one"],
    completedIndependentReviewIds: ["review:one"],
    evidenceIds: ["evidence:story"],
...
test("blocks malformed project and programme identity before normalization", async () => {
  const result = await validateAndNormalizeBookStoryState(minimal({
    projectId: "../unsafe",
    programmeId: "constructor",
    projectFingerprint: "not-a-sha",
  }));
...
test("blocks malformed checkpoint and evidence authority before normalization", async () => {
  const result = await validateAndNormalizeBookStoryState(minimal({
    checkpointId: "__proto__",
    evidenceIds: ["evidence:story", "evidence:story"],
  }));
  assert.equal(result.status, "blocked");

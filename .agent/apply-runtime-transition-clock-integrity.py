...
TEST_PATH = ROOT / "packages/runtime/test/transition-clock-integrity-security.test.mjs"
...
function draft(
""",
    "transition clock helper insertion",
)
...
""",
    """  return Object.freeze({
""",
    "remove former claim clock capture",
)


def transform_method_clock(text: str, method: str, next_method: str) -> str:
    start_marker = f"  public async {method}("
    end_marker = f"  public async {next_method}("
    start = text.index(start_marker)
    end = text.index(end_marker, start)
    block = text[start:end]
    if "snapshotRuntimeTransitionClock(now)" in block:
        raise RuntimeError(f"{method}: transition clock already present")
    body_start = block.index("{\n") + 2
    header = block[:body_start]
    body = block[body_start:]
    if not re.search(r"\bnow\b", body):
        raise RuntimeError(f"{method}: no live clock use found")
    body = re.sub(r"\bnow\b", "transitionNow", body)
    body = "    const transitionNow = snapshotRuntimeTransitionClock(now);\n" + body
...
for method, next_method in [
    ...
    source = transform_method_clock(source, method, next_method)

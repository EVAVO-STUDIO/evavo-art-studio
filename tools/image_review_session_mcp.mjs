...
async function runSession(args) {
  ...
  const receipt = Object.freeze({
    contract: "evavo.image-review-session.v1_1",
    reviewEngineContract: "evavo_image_review_orchestrator_v1_3",
  ...
  return Object.freeze({ ok: true, receiptPath, receiptSha256: sha256(Buffer.from(payload, "utf8")), sourceBinding: receipt.sourceBinding, decision: result.decision, finishingRoute: result.finishingPlan.route, blockers: result.blockers, warnings: result.warnings, visualReviewRequired: true, publicationAllowed: false });
}
...
async function verifySession(args) {
  ...
  if (value.contract !== "evavo.image-review-session.v1_1") throw new Error("Unsupported image-review session receipt contract.");
  if (value.reviewEngineContract !== "evavo_image_review_orchestrator_v1_3") throw new Error("Image-review receipt was produced by an unsupported review engine.");
  ...
  return Object.freeze({ ok: true, receiptPath: receiptFile.path, receiptSha256: receiptFile.sha256, sourceBindingVerified: true, comparisonBindingsVerified: true, finishingPlanVerified: true, decision: value.decision, finishingRoute: value.finishingPlan.route, approvalState: value.approvalState, publicationAllowed: false });
}

const tools = Object.freeze([
  Object.freeze({ name: "evavo_image_review_session_capabilities", description: "Describe durable source-bound unified image-review receipts with preservation-first finishing-plan evidence.", inputSchema: { type: "object", properties: {}, additionalProperties: false } }),
  Object.freeze({ name: "evavo_create_image_review_session", description: "Persist exact-byte-bound unified image QA including the review-only preservation finishing plan. Never grants repair or promotion authority.", inputSchema: { type: "object", properties: { inputPath: { type: "string", minLength: 1 }, receiptPath: { type: "string", minLength: 1 }, intendedRole: { type: "string", enum: [...ROLES] }, declaredProfile: { type: "string", enum: [...IMAGE_REVIEW_PROFILE_NAMES] }, filename: { type: "string" }, compareAgainst: { type: "array", maxItems: 24, items: { type: "object", required: ["id", "path"], properties: { id: { type: "string", minLength: 1 }, path: { type: "string", minLength: 1 } }, additionalProperties: false } }, confirmLocalWrite: { type: "boolean", const: true } }, required: ["inputPath", "receiptPath", "confirmLocalWrite"], additionalProperties: false } }),
  Object.freeze({ name: "evavo_verify_image_review_session", description: "Reverify exact source/comparison bindings plus the governed review-only finishing plan before downstream use.", inputSchema: { type: "object", properties: { receiptPath: { type: "string", minLength: 1 } }, required: ["receiptPath"], additionalProperties: false } }),
]);
function capabilities() {
  return Object.freeze({ contract: "evavo.image-review-session.v1_1", serverVersion: SERVER_VERSION, reviewEngineContract: "evavo_image_review_orchestrator_v1_3", finishingPlanPersisted: true, finishingPlanNeverAuthorizesRepair: true, sourceSha256AndLengthBound: true, comparisonSha256AndLengthBound: true, staleEvidenceVerification: true, createOnlyReceiptWrite: true, sourceMutationPerformed: false, visualReviewRequired: true, publicationAllowed: false, allowedRootCount: configuredLocalRootCount(ALLOWED_ROOTS_ENV), writesEnabled: writesEnabled() });
}

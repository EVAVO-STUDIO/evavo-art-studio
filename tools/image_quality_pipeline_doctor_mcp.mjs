...
const CHECKS = Object.freeze([
  ...
  { id: "work-publication-execution-authorization", file: "tools/work_header_publication_execution_authorization_mcp.mjs", tokens: ['SERVER_VERSION = "1.0.0"', 'CONTRACT = "evavo.work-header-publication-execution-authorization.v1"', 'SCHEMA_SHA256 = "f56a746773e4e08db8e2d22c5bcc0fa50f8416bbfedbc6f8c18c26326ea3dff4"', "confirmExecutionAuthorization=true is required", "currentTargetRecheckRequired: true", "rollbackBackupReverificationRequired: true", "singleTransactionAuthorizationOnly: true", "authorizationExpiresOnAnyEvidenceDrift: true", "evavo_authorize_work_header_publication_execution", "evavo_verify_work_header_publication_execution_authorization", "executionAllowed: false", "publicationAllowed: false", "cloudOverwriteAllowed: false", "websiteMutationAllowed: false"] },
  { id: "work-publication-execution-authorization-schema", file: "contracts/work-header-publication-execution-authorization-v1.schema.json", tokens: ["evavo.work-header-publication-execution-authorization.v1", '"authorized-unexecuted"', '"explicitExecutionConfirmation"', '"executionAuthorizedForOneTransactionOnly"', '"authorizationExpiresOnAnyEvidenceDrift"', '"additionalProperties": false'] },
...
async function inspect() {
  ...
  return Object.freeze({ contract: "evavo.image-quality-pipeline-doctor.v1_21", ready: blockers.length === 0, blockerCount: blockers.length, blockers, checks, executionPerformed: false, sourceMutationPerformed: false, publicationAllowed: false, nextAction: blockers.length ? "Repair failing image-quality contract surfaces." : "Static preservation and exact evidence lineage now extend through browser preview, page review, explicit reviewer approval, publication preparation, rollback-backed transaction planning and a second explicit single-transaction execution authorization; no publication executor is granted by this doctor." });
}
const tools = Object.freeze([
  ...
  { name: "evavo_run_image_quality_pipeline_doctor", description: "Inspect preservation and exact evidence lineage through browser preview, page review, reviewer approval, publication preparation, rollback-backed transaction planning and explicit single-transaction execution authorization without mutating source.", inputSchema: { type: "object", properties: {}, additionalProperties: false } },
]);
const capabilities = () => Object.freeze({ contract: "evavo.image-quality-pipeline-doctor.v1_21", serverVersion: SERVER_VERSION, readOnly: true, browserResponseBodyLineageChecked: true, pageReviewInputSafetyChecked: true, fullPageReviewReceiptLineageChecked: true, approvalPacketReverificationChecked: true, explicitReviewerDecisionBoundaryChecked: true, publicationPreparationBoundaryChecked: true, publicationTransactionPlanBoundaryChecked: true, publicationExecutionAuthorizationBoundaryChecked: true, secondExplicitExecutionConfirmationChecked: true, currentTargetRecheckChecked: true, rollbackBackupEqualityChecked: true, runtimeExecutionPerformed: false, sourceMutationPerformed: false, publicationAllowed: false, checkCount: CHECKS.length });

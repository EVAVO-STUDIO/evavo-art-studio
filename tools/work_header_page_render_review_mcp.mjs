...
function pageReviewSpecFromEvidence(value, preview, bindings) {
  const input = value?.evidence?.reviewInputs;
  if (!input || value?.evidence?.normalizedReviewInputsPersisted !== true) throw new Error("Page-render receipt is missing normalized review inputs required for deterministic reverification.");
  if (!Array.isArray(input.notes)) throw new Error("Page-render reviewInputs.notes is missing.");
  return {
    pageSlug: value.evidence.pageSlug,
    pageTitle: value.evidence.pageTitle,
    candidateId: value.evidence.candidateId,
    candidateSha256: bindings.candidateImage.sha256,
    ...
    titleLegibility: input.titleLegibility,
    focalPointQuality: input.focalPointQuality,
    hierarchyQuality: input.hierarchyQuality,
    responsiveConsistency: input.responsiveConsistency,
    overallPageQuality: input.overallPageQuality,
    currentPageQuality: input.currentPageQuality,
    minimumPageQualityAdvantage: input.minimumPageQualityAdvantage,
    titleObscured: input.titleObscured,
    textContrastFailure: input.textContrastFailure,
    importantSubjectCropped: input.importantSubjectCropped,
    layoutOverflowOrBreakage: input.layoutOverflowOrBreakage,
    candidateLooksWorseThanCurrent: input.candidateLooksWorseThanCurrent,
    notes: input.notes,
  };
}
...
async function runPageReview(args) {
  ...
  if (result.evidence.normalizedReviewInputsPersisted !== true) throw new Error("Page-render core did not persist normalized review inputs.");
...
async function verifyPageReceipt(p) {
  ...
  const recomputed = await reviewWorkHeaderPageRender(pageReviewSpecFromEvidence(value, preview, bindings));
  if (canonicalJson(recomputed.evidence) !== canonicalJson(value.evidence)) throw new Error("Page-render evidence changed after review or no longer recomputes from persisted normalized review inputs.");
  if (sha256(recomputed.proofPng) !== value.proofSha256 || recomputed.proofPng.length !== value.proofByteLength) throw new Error("Page-render proof no longer recomputes from exact screenshots and normalized review inputs.");

  return { path: receipt.path, sha256: receipt.sha256, byteLength: receipt.byteLength, value, selection, preview, pageReviewEvidenceRecomputedAndMatched: true, pageReviewProofRecomputedAndMatched: true, exactTriggeredBrowserRequestBindingVerified: true, fullReceiptLineageVerified: true };
}
...
async function runApprovalPacket(args) {
  ...
  if (page.exactTriggeredBrowserRequestBindingVerified !== true || page.pageReviewEvidenceRecomputedAndMatched !== true || page.pageReviewProofRecomputedAndMatched !== true) throw new Error("Approval packet requires deterministic page-review evidence/proof reverification plus exact triggered Chrome request lineage.");
...
async function verifyApprovalPacketReceipt(p) {
  ...
  if (value.fullReceiptLineageVerified !== true || value.browserResponseBodyIdentityVerified !== true || value.browserResponseMetadataBound !== true || value.exactTriggeredBrowserRequestBindingVerified !== true || value.pageReviewEvidenceRecomputedAndMatched !== true || value.pageReviewProofRecomputedAndMatched !== true) throw new Error("Approval packet receipt lacks fully verified exact browser-request/receipt/page-review lineage.");

  const page = await verifyPageReceipt(value.pageRenderReceiptPath);
  if (page.sha256 !== value.pageRenderReceiptSha256 || page.byteLength !== value.pageRenderReceiptByteLength || page.pageReviewEvidenceRecomputedAndMatched !== true || page.pageReviewProofRecomputedAndMatched !== true) throw new Error("Approval packet is bound to a stale or non-recomputable page-render receipt.");

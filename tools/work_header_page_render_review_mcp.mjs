...
async function verifyApprovalPacketReceipt(p) {
  ...
  if (value.fullReceiptLineageVerified !== true || value.browserResponseBodyIdentityVerified !== true || value.browserResponseMetadataBound !== true) throw new Error("Approval packet receipt lacks fully verified browser/receipt lineage.");
  ...
  for (const profile of ["desktop", "mobile"]) if (!browserBindingEquals(value.browserResponseBindings?.[profile], page.preview.browserResponseBindings?.[profile])) throw new Error(`${profile} approval packet Chrome response metadata drifted from the fully verified page-render lineage.`);

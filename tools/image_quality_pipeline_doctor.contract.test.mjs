...
test("image quality doctor covers review through evidence-only execution result attestation", async () => {
  const source = await read("./image_quality_pipeline_doctor_mcp.mjs");
  for (const token of [
    'contract: "evavo.image-quality-pipeline-doctor.v1_25"',
...
test("doctor requires evidence-only post-execution result verification", async () => {
  const source = await read("./image_quality_pipeline_doctor_mcp.mjs");
  for (const token of [
    ...
    "executionResultEvidenceOnlyChecked: true",
  ]) assert.ok(source.includes(token), `missing execution-result doctor token: ${token}`);
});

test("MCP configuration exposes attestation but no publication executor", async () => {

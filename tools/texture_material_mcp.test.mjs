...
const serverPath = fileURLToPath(new URL("./texture_material_mcp.mjs", import.meta.url));

async function call(root, name, args, { writes = true } = {}) {
  const child = spawn(process.execPath, [serverPath], {
    env: {
      ...process.env,
      EVAVO_TEXTURE_MATERIAL_ALLOWED_ROOTS: root,
      EVAVO_TEXTURE_MATERIAL_ALLOW_WRITES: writes ? "true" : "false",
    },
...
test("reviews a coherent texture set through the MCP without writing", async () => {
  ...
  assert.equal(response.structuredContent.review.width, 4);
  assert.equal(response.structuredContent.review.godot.preferredMaterial, "StandardMaterial3D");
});
...
test("packs ORM create-only and preserves exact scalar texels", async () => {
  ...
  assert.match(repeated.structuredContent.message, /Create-only texture target already exists/);
});
...
test("creates a 3x3 tile proof but refuses writes without explicit admission", async () => {
  ...
  assert.equal(response.isError, false);
  assert.equal(response.structuredContent.evidence.proofWidth, 24);
  assert.equal(response.structuredContent.evidence.proofHeight, 24);

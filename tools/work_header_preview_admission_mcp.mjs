...
function browserResponseBindingsFromManifest(manifest) {
  return {
    desktop: normalizedBrowserBinding(captureByProfile(manifest, "desktop").browserCandidateResponse ?? {}),
    mobile: normalizedBrowserBinding(captureByProfile(manifest, "mobile").browserCandidateResponse ?? {}),
  };
}
...
async function verifyReceipt(receiptPath) {
  ...
  const browserResponseBindings = assertBrowserResponseIdentity(manifest, value.browserResponseBindings);
...
async function admit(args) {
  ...
  const manifest = JSON.parse(manifestFile.bytes.toString("utf8"));
  const browserResponseBindings = assertBrowserResponseIdentity(manifest);

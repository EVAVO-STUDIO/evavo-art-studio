...
{name:'evavo_game_art_sprite_build',description:'Build one create-only sprite atlas, manifest and Godot SpriteFrames package from reviewed frames, bound to the exact plan SHA-256.',inputSchema:obj({workspaceRoot:pathField,planPath:pathField,planSha256:shaField,outputRoot:pathField},['workspaceRoot','planPath','planSha256','outputRoot'])},
];
...
function call(name,a){if(name==='evavo_game_art_workstation_capabilities')return summary({schema:'evavo.game-art-workstation-capabilities.v2',rasterOperations:['crop','trim-alpha','resize','scale-integer','flip','rotate-90','canvas','alpha-threshold','mask-alpha','composite','erase-colour','replace-colour','levels','brightness','contrast','saturation','sharpen','palette-quantize','pixelate','outline'],spriteOutputs:['png-atlas','manifest','godot-spriteframes'],pixelAudit:true,exactPlanShaRequiredForWrites:true});
...
if(name==='evavo_game_art_sprite_build')return summary(runPy('sprite_workstation.py',['--workspace-root',confined(a.workspaceRoot,'workspaceRoot'),'--plan',confined(a.planPath,'planPath'),'--plan-sha256',exactSha(a.planSha256,'planSha256'),'--output-root',confined(a.outputRoot,'outputRoot')]));
...
const rl=createInterface({input:process.stdin,crlfDelay:Infinity});for await(const line of rl){if(!line.trim())continue;let q;try{q=JSON.parse(line);if(q.method==='initialize')reply(q.id,{protocolVersion:'2025-03-26',capabilities:{tools:{}},serverInfo:{name:'evavo-game-art-workstation',version:'1.1.0'}});else if(q.method==='notifications/initialized'){}else if(q.method==='tools/list')reply(q.id,{tools});else if(q.method==='tools/call'){const r=call(q.params?.name,q.params?.arguments??{});reply(q.id,{content:[{type:'text',text:JSON.stringify(r,null,2)}],structuredContent:r,isError:false});}else failReply(q.id,new Error(`Unsupported method ${q.method}`));}catch(e){if(q?.id!==undefined)failReply(q.id,e)}}

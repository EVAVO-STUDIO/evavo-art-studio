...
const LOCATION_VALUE = /^(?:[A-Za-z]:[\\/]|\\\\|\/|\.\.?[\\/]|~[\\/]|[A-Za-z][A-Za-z0-9+.-]*:)/;
const EPSILON = 1e-9;
...
export function assertAnimationDeliveryContainsNoLocations(value, path = "input", depth = 0) {
  ...
  if (Array.isArray(value)) {
    ...
    value.forEach((entry, index) =>
      assertAnimationDeliveryContainsNoLocations(entry, `${path}[${index}]`, depth + 1),
    );
    return;
  }
  ...
  for (const [key, entry] of entries) {
    ...
    assertAnimationDeliveryContainsNoLocations(entry, `${path}.${key}`, depth + 1);
  }
}
...
export function assertAnimationSequenceDeliverySemanticIntegrity(delivery) {
  base.assertAnimationSequenceDeliveryIntegrity(delivery);
  assertAnimationDeliveryContainsNoLocations(delivery);
  ...
  if (delivery.targets.godot) {
    ...
    const targetDurations = delivery.targets.godot.frames.map((entry) => entry.relativeDuration);
    if (JSON.stringify(targetDurations) !== JSON.stringify(expectedDurations)) fail("ANIMATION_DELIVERY_GODOT_DURATIONS_INVALID");
  }
  ...
  if (delivery.targets.video) {
    ...
    const entries = delivery.targets.video.entries;
    let cursor = 0;
...
export function assertAnimationProductionProfileForDelivery(profile) {
  assertAnimationDeliveryContainsNoLocations(profile);
...
export function assertAcceptedAnimationReview(profile, decision) {
  assertAnimationDeliveryContainsNoLocations(profile);
  assertAnimationDeliveryContainsNoLocations(decision);
...
export function assertAnimationSequenceCreativeApproval(profile, decision, artifacts, approval) {
  assertAnimationDeliveryContainsNoLocations(profile);
  assertAnimationDeliveryContainsNoLocations(decision);
  assertAnimationDeliveryContainsNoLocations(artifacts);
  assertAnimationDeliveryContainsNoLocations(approval);
...
export function compileAnimationSequenceDelivery(input, now = new Date()) {
  assertAnimationDeliveryContainsNoLocations(input);
  const delivery = base.compileAnimationSequenceDelivery(input, now);
  assertAnimationSequenceDeliverySemanticIntegrity(delivery);
  return delivery;
}

export function assertAnimationSequenceDeliveryIntegrity(delivery) {
  return assertAnimationSequenceDeliverySemanticIntegrity(delivery);
}
...
export function compileVideoStudioAnimationIntake(delivery, now = new Date()) {
  ...
  assertVideoStudioAnimationIntakeIntegrity(intake);
  return intake;
}

export function assertVideoStudioAnimationIntakeIntegrity(intake) {
  assertAnimationDeliveryContainsNoLocations(intake);
  ...
  const duration = intake.entries.reduce((sum, entry) => sum + requirePositiveFinite(entry.durationSeconds, "ANIMATION_DELIVERY_VIDEO_INTAKE_DURATION_INVALID", 120), 0);
  if (Math.abs(duration - intake.totalDurationSeconds) > EPSILON) fail("ANIMATION_DELIVERY_VIDEO_INTAKE_TOTAL_DURATION_INVALID");
}
...
async function cli() {
  ...
  if (!command || !inputPath || !["compile", "verify", "video", "verify-video"].includes(command)) {
    fail("ANIMATION_DELIVERY_USAGE", "node tools/animation_sequence_delivery_v1_hardened.mjs <compile|verify|video|verify-video> <input.json> [output.json]");
  }
  ...
  if (command === "verify") {
    assertAnimationSequenceDeliverySemanticIntegrity(input);

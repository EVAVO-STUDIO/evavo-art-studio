...
async function runReview(args) {
  ...
  const receipt = {
    contract: "evavo.enhancement-art-review-session.v1_3",
  ...
  return { ok: true, receiptPath, qualityProofPath, differenceProofPath, pageProofPath, imageReviewSessionReceiptPath: imageReviewSession.path, imageReviewSessionReceiptSha256: imageReviewSession.sha256, immutableSourceComparisonBindingVerified: true, finishingPlanVerified: true, finishingRoute: imageReviewSession.value.finishingPlan.route, evidence: result.evidence };
}

const tools = [
  { name: "evavo_enhancement_review_session_capabilities", description: "Describe end-to-end enhancement review with durable candidate/source and finishing-plan lineage.", inputSchema: { type: "object", properties: {}, additionalProperties: false } },
  { name: "evavo_review_enhancement_candidate_end_to_end", description: "Verify exact candidate/source bytes and a current durable image-review receipt including the review-only finishing plan, then run source-space/page-context enhancement review and write rollback-safe evidence.", inputSchema: { type: "object", properties: { manifestPath: { type: "string", minLength: 1 }, imageReviewSessionReceiptPath: { type: "string", minLength: 1 }, outputPrefix: { type: "string", minLength: 1 }, headerPath: { type: "string" }, supportPath: { type: "string" }, tilePath: { type: "string" }, desktopScreenshotPath: { type: "string" }, mobileScreenshotPath: { type: "string" }, confirmLocalWrite: { type: "boolean" } }, required: ["manifestPath", "imageReviewSessionReceiptPath", "outputPrefix", "confirmLocalWrite"], additionalProperties: false } },
];
function capabilities() {
  return { contract: "evavo.enhancement-art-review-session.v1_3", serverVersion: SERVER_VERSION, allowedRootCount: configuredLocalRootCount(ROOTS_ENV), writesEnabled: writesEnabled(), durableImageReviewSessionRequired: true, requiredImageReviewSessionContract: "evavo.image-review-session.v1_1", requiredImageReviewEngineContract: "evavo_image_review_orchestrator_v1_3", exactCandidateReviewSessionBindingRequired: true, immutableSourceComparisonBindingRequired: true, finishingPlanRequired: true, finishingPlanMustRemainReviewOnly: true, imageReviewSessionProfileMustMatchManifest: true, staleImageReviewSessionRejected: true, rollbackSafeReviewArtifactBundle: true, publicationAllowed: false, cloudOverwriteAllowed: false, automaticCreativeApproval: false };
}

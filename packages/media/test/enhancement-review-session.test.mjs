...
function manifest(source, candidate, width, height, role = "illustration", profile = "illustration") {
  return {
...
test("end-to-end enhancement review never grants publishing authority", async () => {
  ...
  const result = await reviewEnhancementStudioCandidate({
    manifest: manifest(source, candidate, 320, 180),
    source,
    candidate,
  });
...
test("rejects candidate bytes that do not match manifest", async () => {
  ...
  await assert.rejects(
    () => reviewEnhancementStudioCandidate({
      manifest: manifest(source, candidate, 64, 64),
      source,
      candidate: wrong,
    }),

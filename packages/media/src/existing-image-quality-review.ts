      ...
      if (a <= 32 && x > 0 && y > 0 && x < width - 1 && y < height - 1) {
        ...
        for (let yy = y - 1; yy <= y + 1; yy += 1) {
          for (let xx = x - 1; xx <= x + 1; xx += 1) {
            if (xx === x && yy === y) continue;
            const ni = sampleOffset(width, xx, yy);
            if (rgba[ni + 3]! >= 223) opaqueNeighbours += 1;
          }
        }
...
export async function reviewExistingImageQuality(
  encoded: Buffer,
  spec: ExistingImageQualitySpec = {},
): Promise<ExistingImageQualityEvidence> {
  if (encoded.byteLength === 0) throw new Error("Image quality review input is empty.");
  const decoded = await rawRgba(encoded);
  const metrics = analysePixels(decoded.width, decoded.height, decoded.raw);
  ...
  if (metrics.shadowClipRatio > 0.45) {
    issues.push(`heavy-shadow-clipping:${(metrics.shadowClipRatio * 100).toFixed(2)}%`);
    score -= 8;
  }
  if (metrics.highlightClipRatio > 0.25) {
    issues.push(`heavy-highlight-clipping:${(metrics.highlightClipRatio * 100).toFixed(2)}%`);
    score -= 8;
  }
  if (metrics.transparentRgbContaminationRatio > maximumTransparentRgbContaminationRatio) {
    issues.push(`dirty-transparent-rgb:${(metrics.transparentRgbContaminationRatio * 100).toFixed(4)}%`);
    score -= 18;
  }
  if (metrics.edgeHaloRiskRatio > maximumEdgeHaloRiskRatio) {
    issues.push(`edge-halo-risk:${(metrics.edgeHaloRiskRatio * 100).toFixed(2)}%`);
    score -= 22;
  }
  if (metrics.alphaPinholeRatio > maximumPinholeRatio) {
    issues.push(`alpha-pinholes:${(metrics.alphaPinholeRatio * 100).toFixed(4)}%`);
    score -= 14;
  }
  if (metrics.blockinessRatio > maximumBlockinessRatio) {
    issues.push(`jpeg-blockiness-risk:${metrics.blockinessRatio.toFixed(2)}x`);
    score -= 12;
  }
...
async function labelledPreview(encoded: Buffer, label: string, background: string): Promise<Buffer> {
  ...
  return sharp({ create: { width, height: height + 42, channels: 4, background: "#080808" } })
    .composite([{ input: preview, left: 0, top: 0 }, { input: labelSvg, left: 0, top: height }])
    .png()
    .toBuffer();
}

export async function reviewExistingImageEdit(
  sourceEncoded: Buffer,
  editedEncoded: Buffer,
  spec: ExistingImageEditReviewSpec = {},
): Promise<ExistingImageEditReviewResult> {
  ...
  const verdict: ExistingImageQualityGrade = regressions.length > 0 || edited.grade === "fail"
    ? "fail"
    : edited.grade === "warn"
      ? "warn"
      : "pass";

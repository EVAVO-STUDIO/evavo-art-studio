...
const ROUTES: Readonly<Record<ImageAgentGoal, Omit<ImageAgentRoute, "contract" | "goal">>> = Object.freeze({
  ...
  "reference-consistency": Object.freeze({
    ...
    orderedTools: Object.freeze(["evavo_review_image_against_references", "evavo_review_image_batch_against_references", "evavo_create_image_reference_consistency_proof"]),
    writeClass: "read-only",
    ...
    evidenceExpected: Object.freeze(["reference-set coherence", "palette/tone/detail/silhouette/framing distances", "nearest approved references", "ranked batch outliers", "optional visual proof for human review"]),
    notes: Object.freeze([
      ...
      "Review calls are read-only; the optional diagnostic proof is a separately write-gated create-only operation.",
  ...
  "ai-artifact-assessment": Object.freeze({
    primarySurface: "evavo-image-artifact-triage + evavo-image-reference-consistency + provenance/semantic review",
    orderedTools: Object.freeze([
      "evavo_review_image_artifact_risk",
      "evavo_review_image_against_references",
      "trusted provenance/lineage review",
      "human semantic visual review",
    ]),
    writeClass: "read-only",
    prerequisites: Object.freeze(["actual image", "approved references when style/identity consistency matters", "trusted provenance/source lineage when authorship matters"]),
    stopConditions: Object.freeze([
      "do not infer authorship from pixel heuristics",
      "reference set is unstable",
      "artifact signal requires semantic inspection rather than deterministic conclusion",
    ]),
    evidenceExpected: Object.freeze([
      "ringing/posterization/resampling evidence",
      "repeated nontrivial detail and local-detail-density evidence",
      "quality/defect context",
      "optional approved-reference consistency evidence",
      "provenance status",
      "human semantic verdict for anatomy/text/identity/reflections/perspective/content",
    ]),
    notes: Object.freeze([
      "Art Studio identifies production defects and suspicious visual signals, not image authorship.",
      "Repeated patterns or uneven detail may also come from cloning, tiling, compression or intentional art direction.",
      "Use provenance records to answer origin questions and human/vision review for semantic defects that statistics cannot establish.",
    ]),
  }),

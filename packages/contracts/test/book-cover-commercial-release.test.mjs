...
function baseInput() {
  const requiredProofIds = ["thumbnail_60px", "grayscale", "blur_squint", "full_size"];
  return {
    ...
    designIntelligence: {
      ...
      direction: {
        ...
        retailProofPlan: {
          requiredProofs: requiredProofIds.map((proofId) => ({ proofId, passCondition: "Passes exact proof condition.", humanDecisionRequired: true })),
        },
    ...
    proofResults: requiredProofIds.map((proofId) => ({
      proofId,
      status: "pass",
...
test("authorizes only a fully evidenced local-first Docs Suite handoff", () => {
  const result = compileBookCoverCommercialReleaseAuthority(baseInput());
  ...
  assert.deepEqual(validateBookCoverCommercialReleaseAuthority(result.authority), { valid: true, issues: [] });
});

test("is deterministic for identical retained evidence", () => {
  const first = compileBookCoverCommercialReleaseAuthority(baseInput());
  const second = compileBookCoverCommercialReleaseAuthority(baseInput());
...
test("detects authority tampering", () => {
  ...
  const validation = validateBookCoverCommercialReleaseAuthority(tampered);

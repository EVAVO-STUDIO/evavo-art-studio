...
export async function compileTileMapCandidateReview(
  ...
  if (results.schema_version !== 1) {
    throw failure(
      ...
      "candidate results schema_version must be 1",
    );
  }

  const batchFingerprint = sha256Hex(
  ...
  if (
    ...
    throw failure(
      ...
      "candidate results do not preserve authorized provider intermediate/review-only authority",
    );
  }
  ...
  for (const [index, item] of resultRows.entries()) {
    ...
    if (!job) {
      throw failure(
        ...
        `unknown candidate result ${candidateId}`,
      );
    }
    ...
    if (actualSha !== expectedSha) {
      throw failure(
        ...
        `${candidateId} result hash changed`,
      );
    }
    if (seenDigests.has(actualSha)) {
      throw failure(
        "EVAVO_TILE_MAP_REVIEW_DUPLICATE",
        `${candidateId} duplicates another candidate's exact bytes`,
      );
    }
    ...
    if (metadata.format !== "png" || !metadata.width || !metadata.height) {
      throw failure(
        ...
        `${candidateId} must be a readable PNG`,
      );
    }
    ...
    if (
      ...
      throw failure(
        ...
        `${candidateId} is ${metadata.width}x${metadata.height}; expected ${expectedWidth}x${expectedHeight}`,
      );
    }

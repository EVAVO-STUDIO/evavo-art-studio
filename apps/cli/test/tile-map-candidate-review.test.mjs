...
async function fixture({ duplicate = false, wrongFingerprint = false, wrongSize = false } = {}) {
  ...
  for (let index = 0; index < 2; index += 1) {
    ...
    const bytes = duplicate && index === 1
      ? await import("node:fs/promises").then(({ readFile }) => readFile(path.join(root, "candidates", "grass", "01.png")))
      : await sharp({ create: { width: wrongSize && index === 0 ? 17 : 16, height: 16, channels: 4, background: { r: 30 + index * 20, g: 120, b: 60, alpha: 1 } } }).png().toBuffer();
    ...
    candidates.push({ candidate_id: candidateId, path: output, sha256: sha(bytes) });
  }
  const batch = {
    schema_version: 1,
  ...
  const results = {
    schema_version: 1,
    source_batch_fingerprint: wrongFingerprint ? "e".repeat(64) : "d".repeat(64),
    candidates,
  };
...
test("admits exact provider outputs into review with all approval states pending", async () => {
  const input = await fixture();
  const result = await compileTileMapCandidateReview(input.batchPath, input.resultsPath);

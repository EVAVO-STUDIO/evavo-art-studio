...
test("MCP exposes layered source planning without provider or promotion authority", async () => {
  ...
  for (const token of [
    ...
    "no image generation, composite assembly, approval",
    "Candidate count remains one",
  ]) {
    assert.ok(tools.includes(token), `missing layered-production MCP invariant: ${token}`);
  }
  for (const forbidden of [
    ...
    assert.ok(!tools.includes(forbidden), `layered-production MCP contains execution shortcut: ${forbidden}`);
  }
});

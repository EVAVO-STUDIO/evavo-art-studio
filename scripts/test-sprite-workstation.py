...
class SpriteWorkstationTests(unittest.TestCase):
    def build_fixture(self,r,with_reserved=False,sequence=None):
        frames=[]
    ...
    def test_authored_sequence_preserves_repeated_cells_and_order(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);_,plan_path=self.build_fixture(r,False,[0,1,2,1]);result=self.run_plan(r,plan_path)
            self.assertEqual(result.returncode,0,result.stderr);tres=(r/'out'/'sprite_frames.tres').read_text()
            tokens=['AtlasTexture_0000','AtlasTexture_0001','AtlasTexture_0002','AtlasTexture_0001']
            positions=[];start=tres.index('"name": &"run"')-500
            animation=tres[start:]
            cursor=0
            for token in tokens:
                pos=animation.find(token,cursor);self.assertGreaterEqual(pos,0,animation);positions.append(pos);cursor=pos+1
    ...
    def test_sequence_cannot_reference_reserved_cell(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);_,plan_path=self.build_fixture(r,True,[0,1,2]);result=self.run_plan(r,plan_path)

...
class SpriteWorkstationTests(unittest.TestCase):
    def build_fixture(self,r,with_reserved=False,sequence=None,explicit_cells=False):
        frames=[]
        for i,c in enumerate(((255,0,0,255),(0,0,255,255),(0,255,0,255))):
            p=r/f'f{i}.png';p.write_bytes(png(c));row={'id':f'run_{i}','animation':'run','path':p.name,'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'durationMs':125}
            if explicit_cells:row['cellIndex']=i
            frames.append(row)
        if with_reserved:
            frames.insert(1,{'id':'reserved_1','animation':'reserved','reserved':True,'runtime':False})
            if explicit_cells:
                for i,row in enumerate(frames):row['cellIndex']=i
        animations={'run':{'fps':8,'loop':True}}
        if sequence is not None:animations['run']['sequence']=sequence
        plan={'schema':'evavo.sprite-workstation-plan.v1','repositoryMutation':False,'automaticApproval':False,'cell':{'width':8,'height':8},'columns':2,'hardAlpha':True,'frames':frames,'animations':animations,'godotAtlasPath':'res://assets/run.png'}
    ...
    def test_build(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);_,plan_path=self.build_fixture(r,explicit_cells=True);result=self.run_plan(r,plan_path)
            ...
            manifest=json.loads((r/'out'/'atlas.json').read_text());self.assertEqual(manifest['schema'],'evavo.sprite-workstation-manifest.v2');self.assertEqual(manifest['planSha256'],receipt['planSha256']);self.assertEqual([row['cellIndex'] for row in manifest['frames']],[0,1,2])
    ...
    def test_authored_sequence_preserves_repeated_cells_and_order(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);_,plan_path=self.build_fixture(r,False,[0,1,2,1],True);result=self.run_plan(r,plan_path)
            self.assertEqual(result.returncode,0,result.stderr);tres=(r/'out'/'sprite_frames.tres').read_text();animation=tres[tres.index('[resource]'):]
            tokens=['AtlasTexture_0000','AtlasTexture_0001','AtlasTexture_0002','AtlasTexture_0001'];cursor=0
            for token in tokens:
                pos=animation.find(token,cursor);self.assertGreaterEqual(pos,0,animation);cursor=pos+len(token)
    def test_reserved_cell_preserves_index_and_is_not_runtime_animation(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);_,plan_path=self.build_fixture(r,True,explicit_cells=True);result=self.run_plan(r,plan_path)
    ...
    def test_sequence_cannot_reference_reserved_cell(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);_,plan_path=self.build_fixture(r,True,[0,1,2],True);result=self.run_plan(r,plan_path)
            self.assertEqual(result.returncode,2);self.assertIn('non-runtime cell 1',result.stderr)
    def test_declared_cell_index_cannot_differ_from_physical_slot(self):
        with tempfile.TemporaryDirectory() as d:
            r=Path(d);plan,plan_path=self.build_fixture(r,explicit_cells=True);plan['frames'][1]['cellIndex']=7;plan_path.write_text(json.dumps(plan));result=self.run_plan(r,plan_path)
            self.assertEqual(result.returncode,2);self.assertIn('physical atlas slot 1',result.stderr)

...
function exportedString(relative, name) {
  const match = new RegExp(`export const ${name}\\s*=\\s*"([^"]+)"`, "u").exec(source.get(relative));
...
const implementation = source.get("scripts/layered-godot-git-operator.mjs");
for (const token of [
  "snapshotJsonValue",
  "hash-object",
  "update-index",
...
for (const forbidden of [
  ...
  "--force-with-lease",
  "shell: true",
]) assert.equal(implementation.includes(forbidden), false, `Git operator contains forbidden ${forbidden}`);

const config = JSON.parse(source.get("config/layered-production-godot-git-operator.v1.json"));
...
assert.equal(config.requirements.requiresIndexPreimageRollbackOnSafePreCommitFailure, true);
assert.equal(config.requirements.refusesRollbackOnConcurrentSamePathIndexDrift, true);
...
const tests = spawnSync(process.execPath, ["--test", path.join(root, "scripts/test-layered-godot-git-operator.mjs")], {
  cwd: root,
  encoding: "utf8",
  shell: false,
  windowsHide: true,

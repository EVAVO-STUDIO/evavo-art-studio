...
async function main() {
  ...
  if (!authorizationPath || process.argv.length !== 3) {
    throw new Error('usage: validate-tile-map-provider-authorization.mjs <authorization.json>');
  }
  const record = await readJson(authorizationPath, 'Tile Map provider authorization');
  ...
  const source = await readJson(authorization.sourceProviderBatch.path, 'Tile Map provider batch');
  if (sha256(source.bytes) !== authorization.sourceProviderBatch.fileSha256) {
    throw new Error('provider batch bytes drifted');
  }
  if (source.value.provider_batch_fingerprint !== authorization.sourceProviderBatch.documentSha256) {
    throw new Error('provider batch fingerprint drifted');
  }
  if (source.value.source_map_fingerprint !== authorization.sourceMapFingerprint) {
  ...
  for (const expected of authorization.jobs) {
    ...
    if (!job) throw new Error(`authorized runtime job missing: ${expected.jobId}`);
    if (job.specHash !== expected.specSha256) throw new Error(`runtime job spec drifted: ${expected.jobId}`);
    if (job.state !== 'queued' || job.attempts.length !== 0 || job.redriveCount !== 0) {
      throw new Error(`runtime job is not pristine queued state: ${expected.jobId}`);
    }
    if (job.spec.maximumAttempts !== 1) throw new Error(`runtime job retry policy drifted: ${expected.jobId}`);
    if (!job.spec.requiredCapabilities.includes(TILE_MAP_PROVIDER_EXECUTION_CAPABILITY)) {
      throw new Error(`runtime job authorization capability missing: ${expected.jobId}`);
    }
    if (!authorization.execution.queues.includes(job.spec.queue)) {
      throw new Error(`runtime job queue drifted: ${expected.jobId}`);
    }
    results.push({ jobId: job.id, specSha256: job.specHash, queue: job.spec.queue, state: job.state });
  }
  process.stdout.write(`${JSON.stringify({
    status: 'valid',
    authorization: record.path,
    authorizationSha256: expectedHash,
    activeNow: Date.now() >= Date.parse(authorization.authorizedAt) && Date.now() < Date.parse(authorization.expiresAt),

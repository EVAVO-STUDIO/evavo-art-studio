...
const adult = await load("contracts/creative-adult-content-profile-v1.json");
...
assert.equal(adult.kind, "evavo-creative-adult-content-profile-v1");
for (const profile of ["adult-pinup","adult-artistic-nudity","adult-erotic-nonexplicit","adult-strip-game","adult-sensual-animation"]) assert.ok(adult.projectClasses.includes(profile));
assert.equal(adult.workflowRules.genericNsfwBlockerRequired, false);
assert.equal(adult.workflowRules.allDepictedSexualizedCharactersMustBeAdults, true);
assert.equal(adult.workflowRules.sexualizedMinorOrAmbiguousMinorAllowed, false);
assert.equal(adult.workflowRules.nonConsensualSexualScenarioAllowed, false);
assert.equal(adult.runtime.localOpenWeightRoutesPreferredWhenHostedProviderConflicts, true);
...
const adultProfiles = new Map(quality.adultQualityProfiles.map((profile) => [profile.contentProfile, profile]));
for (const profileId of ["adult-pinup","adult-artistic-nudity","adult-strip-game"]) assert.ok(adultProfiles.has(profileId));
assert.deepEqual(adultProfiles.get("adult-artistic-nudity").preferredRouteOrder.slice(0,2), ["realism-challenger","workstation-default"]);
for (const metric of ["adult-age-evidence","adult-anatomy","body-proportions","skin-and-material-quality","reference-adherence"]) assert.ok(adultProfiles.get("adult-artistic-nudity").requiredReview.includes(metric));
for (const metric of ["identity-across-wardrobe-states","body-proportion-consistency","pose-registration","alpha-quality","game-readability"]) assert.ok(adultProfiles.get("adult-strip-game").requiredReview.includes(metric));
for (const signal of ["multi-reference","pose","segmentation"]) assert.ok(adultProfiles.get("adult-strip-game").requiredControls.includes(signal));

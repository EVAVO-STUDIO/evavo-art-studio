...
foreach ($CreateOnlyPath in @($ExecutionReceipt, $Review)) {
    if (Test-Path -LiteralPath $CreateOnlyPath) {
        throw "Create-only output already exists: $CreateOnlyPath"
    }
}
if (Test-Path -LiteralPath $CandidateRoot) {
    $Existing = @(Get-ChildItem -LiteralPath $CandidateRoot -Force)
    if ($Existing.Count -gt 0) {
        throw "Candidate result directory must be absent or empty: $CandidateRoot"
    }
}
...
try {
    ...
    Write-Host ""
    Write-Host 'AUTHORIZED TILE MAP PROVIDER RUN COMPLETE'

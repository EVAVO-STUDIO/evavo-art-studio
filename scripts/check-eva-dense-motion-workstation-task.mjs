...
const here=fileURLToPath(import.meta.url);const root=path.resolve(path.dirname(here),"..");
...
export function validateTask(task,scriptSource,v5Client,preflightSource=""){
  ...
  fail(task.sourceRepository==="EVAVO-STUDIO/evavo-avatar-runtime","EVA source repository drifted.");
  ...
  const requiredMarkers=["Set-StrictMode -Version Latest","$ErrorActionPreference = 'Stop'","$global:LASTEXITCODE = 0","& $Git 'rev-parse' 'HEAD'","& $Git 'status' '--porcelain=v1' '--untracked-files=all'","evavo-avatar-runtime","scripts/project-art/eva-dense-motion-source-preflight.mjs","sourceMediaPreflight = 'passed'","scripts/check-project-art-eva-dense-motion-work-order.mjs","scripts/check-art-studio-workstation-v5-contract.mjs","scripts/test-art-studio-workstation-v5-contract.mjs","providerExecution = $false","cloudinaryUpload = $false","repositoryCommit = $false","repositoryPush = $false","publication = $false","runtimeActivation = $false","forcePush = $false"];
  for(const marker of requiredMarkers)fail(scriptSource.includes(marker),`Tracked PowerShell task is missing: ${marker}`);
  for(const marker of ["EVA_DENSE_MOTION_SOURCE_PREFLIGHT_SCHEMA","gitBlobSha1","inspectPngHeader","EVA_DENSE_SOURCE_GIT_BLOB_MISMATCH","exactSourceIdentityVerified:true","sourceMutation:false","providerExecution:false","candidatePromotion:false","runtimeActivation:false"])fail(preflightSource.includes(marker),`Source preflight is missing: ${marker}`);
  const lower=scriptSource.toLowerCase();
  for(const token of ["Invoke-Expression","-EncodedCommand","git push","git commit","git reset --hard","git clean","Remove-Item","cloudinary upload","candidatePromotion = $true","providerExecution = $true","runtimeActivation = $true"])fail(!lower.includes(token.toLowerCase()),`Tracked PowerShell task contains forbidden material: ${token}`);
  return Object.freeze({schema:"evavo.eva-dense-motion-workstation-task-check.v2",ok:true,family:task.denseMotionFamily,pendingOrdinalCount:task.pendingOrdinals.length,sourcePreflightRequired:true,plannerBound:true,physicalAcceptanceRequired:true,minimumLocalStorageVersion:task.minimumLocalStorageVersion,repositoryWriteAuthority:false,publicationAuthority:false});
}
if(process.argv[1]&&path.resolve(process.argv[1])===path.resolve(here)){const task=json("config/eva-dense-motion-workstation-task-v1.json");const script=read("scripts/Invoke-EvaDenseMotionWorkstationValidation.ps1");const v5=json("config/automation-fabric-client-v5.json");const preflight=read("scripts/project-art/eva-dense-motion-source-preflight.mjs");console.log(JSON.stringify(validateTask(task,script,v5,preflight)));}

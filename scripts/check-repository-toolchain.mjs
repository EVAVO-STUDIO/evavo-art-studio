...
if (!checkCommand.startsWith("pnpm run toolchain:check && pnpm run toolchain:test && ")) {
  errors.push("package.json check must begin with exact toolchain and adversarial validation");
}
...
if (fs.existsSync(resolveInside("pnpm-lock.yaml"))) {
  errors.push("pnpm-lock.yaml appeared before the review-first lockfile transition was approved");
}
...
console.log("- the absent lockfile remains an explicit review-first transition");

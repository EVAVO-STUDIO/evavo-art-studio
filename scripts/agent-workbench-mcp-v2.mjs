...
const tools = Object.freeze([
  ...
  { name: "evavo_agent_workbench_health_fleet", description: "Aggregate read-only Workbench health across the local EVAVO workspace. Partial local coverage is never reported ready, never proves provider completeness or absence, and no repair is performed.", inputSchema: objectSchema({ workspaceRoot: pathField }) },
...
function callTool(name, input) {
  ...
  if (name === "evavo_agent_workbench_health_fleet") return inspectAgentWorkbenchHealthFleet(repositoryRoot, { workspaceRoot: workbenchWorkspaceRoot(input.workspaceRoot) });
...
for await (const line of input) {
  ...
  try {
    ...
    if (request.method === "server/discover") response(request.id, { protocolVersion: EVAVO_DISCOVERY_PROTOCOL, supportedVersions: [STANDARD_PROTOCOL, EVAVO_DISCOVERY_PROTOCOL], serverInfo: { name: "evavo-agent-workbench", version: "2.2.0" }, capabilities: { tools: { count: tools.length, names: tools.map((tool) => tool.name), listChanged: false }, guidance: true, handoff: true, fleet: true, sharedDrift: true, healthFleet: true }, readOnly: true, truthBoundary: { readOnly: true, executionGranted: false, mutationGranted: false, publicationGranted: false, completionClaimProven: false, sharedDriftAutomaticRepair: false, sharedDriftUnavailableDoesNotProveAlignment: true, healthFleetAutomaticRepair: false, healthFleetClaimsProviderCompleteness: false, healthFleetAllowsAbsenceClaims: false } });
    else if (request.method === "initialize") response(request.id, { protocolVersion: STANDARD_PROTOCOL, capabilities: { tools: {} }, serverInfo: { name: "evavo-agent-workbench", version: "2.2.0" } });

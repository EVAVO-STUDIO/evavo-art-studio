...
export function inspectEvaTalkNeutralCandidatePng(bytesInput) {
  ...
  while (offset < bytes.length) {
    assert(
      ...
      'EVA_TALK_NEUTRAL_QUEUE_PNG_TRUNCATED',
    );
    const length = bytes.readUInt32BE(offset);
    const type = bytes.subarray(offset + 4, offset + 8).toString('ascii');
    assert(
      /^[A-Za-z]{4}$/u.test(type),
      'EVA_TALK_NEUTRAL_QUEUE_PNG_CHUNK_TYPE_INVALID',
    );
    if (/^[A-Z]/u.test(type)) {
      assert(
    ...
    assert(
      expectedCrc === observedCrc,
      'EVA_TALK_NEUTRAL_QUEUE_PNG_CRC_INVALID',
    );
    ...
    if (type === 'IHDR') {
      assert(
        ihdr === null && length === 13 && chunkCount === 1,
        'EVA_TALK_NEUTRAL_QUEUE_PNG_IHDR_INVALID',
      );
      ihdr = Object.freeze({
      ...
      assert(
        length === 0 && !iendSeen,
        'EVA_TALK_NEUTRAL_QUEUE_PNG_IEND_INVALID',
      );
      iendSeen = true;
      offset = dataEnd + 4;
      break;
  ...
  assert(
    iendSeen && offset === bytes.length,
    'EVA_TALK_NEUTRAL_QUEUE_PNG_TRAILING_BYTES',
  );
  ...
  try {
    inflated = inflateSync(Buffer.concat(idat), {
      maxOutputLength:
        (EVA_TALK_NEUTRAL_CANVAS.width * 4 + 1) *
        EVA_TALK_NEUTRAL_CANVAS.height,
    });
  } catch {
  ...
  const expectedInflated =
    ...
    EVA_TALK_NEUTRAL_CANVAS.height;
  assert(
    inflated.length === expectedInflated,
    'EVA_TALK_NEUTRAL_QUEUE_PNG_SCANLINES_INVALID',
  );
  const stride = EVA_TALK_NEUTRAL_CANVAS.width * 4 + 1;
  for (let row = 0; row < EVA_TALK_NEUTRAL_CANVAS.height; row += 1) {
    assert(
      inflated[row * stride] <= 4,
      'EVA_TALK_NEUTRAL_QUEUE_PNG_FILTER_INVALID',
    );
  }
  return deepFreeze({
    width: ihdr.width,

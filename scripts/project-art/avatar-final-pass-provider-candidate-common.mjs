...
export class ProjectArtAvatarProviderCandidateError extends Error {
  constructor(code, message = code) {
    super(message);
    this.name = 'ProjectArtAvatarProviderCandidateError';
    this.code = code;
  }
}

export function fail(code, message = code) {
  throw new ProjectArtAvatarProviderCandidateError(code, message);
}
...
export function exactKeys(value, keys, label, code = 'AVATAR_PROVIDER_CANDIDATE_KEYS_INVALID') {
...
export function identifier(value, label) {
  assert(
    ...
    'AVATAR_PROVIDER_CANDIDATE_IDENTIFIER_INVALID',
    `${label} is invalid.`,
  );
  return value;
}

export function sourceCommit(value, label) {
  assert(
    typeof value === 'string' && SHA1_PATTERN.test(value),
    'AVATAR_PROVIDER_CANDIDATE_SOURCE_COMMIT_INVALID',
    `${label} is invalid.`,
  );
  return value;
}
...
export function digest(value, label) {
  assert(
    ...
    'AVATAR_PROVIDER_CANDIDATE_SHA256_INVALID',
    `${label} is invalid.`,
  );
  return value;
}
...
export function artifactId(value, label) {
  assert(
    ...
    'AVATAR_PROVIDER_CANDIDATE_ARTIFACT_ID_INVALID',
    `${label} must use artifact_<sha256> format.`,
  );
  return value;
}

export function providerRequestId(value, label) {
  assert(
    typeof value === 'string' && PROVIDER_REQUEST_ID_PATTERN.test(value),
    'AVATAR_PROVIDER_CANDIDATE_PROVIDER_REQUEST_ID_INVALID',
    `${label} is invalid.`,
  );
  return value;
}
...
export function boundedText(value, label, minimum = 1, maximum = 32_000) {
  assert(
    typeof value === 'string' &&
      ...
      !value.includes('\0'),
    'AVATAR_PROVIDER_CANDIDATE_TEXT_INVALID',
    `${label} is invalid.`,
  );
  return value;
}
...
export function timestamp(value, label) {
  assert(
    ...
    'AVATAR_PROVIDER_CANDIDATE_TIMESTAMP_INVALID',
...
function snapshotValue(value, state, label, depth) {
  assert(
    depth <= MAXIMUM_DEPTH,
    'AVATAR_PROVIDER_CANDIDATE_SNAPSHOT_DEPTH_EXCEEDED',
    `${label} exceeds the maximum nesting depth.`,
  );
  state.nodes += 1;
  assert(
    state.nodes <= MAXIMUM_NODES,
    'AVATAR_PROVIDER_CANDIDATE_SNAPSHOT_SIZE_EXCEEDED',
  ...
  if (typeof value === 'number') {
    assert(
      ...
      'AVATAR_PROVIDER_CANDIDATE_JSON_INVALID',
...
export function verifySelfHash(value, field, label) {
  ...
  assert(
    ...
    'AVATAR_PROVIDER_CANDIDATE_SELF_HASH_MISMATCH',
    `${label}.${field} does not match canonical content.`,
  );
  return deepFreeze(snapshot);
}

export function allFalseAuthority(value, label) {
  assert(isRecord(value), 'AVATAR_PROVIDER_CANDIDATE_AUTHORITY_INVALID', `${label} must be an object.`);
  for (const [key, entry] of Object.entries(value)) {
    assert(
      entry === false,
      'AVATAR_PROVIDER_CANDIDATE_FALSE_AUTHORITY_REQUIRED',
...
export function stableJsonFile(filePath, label) {
  ...
  assert(
    ...
    'AVATAR_PROVIDER_CANDIDATE_INPUT_FILE_INVALID',
    `${label} must be a single-link regular file.`,
  );
  assert(
    before.size >= 2 && before.size <= MAXIMUM_DOCUMENT_BYTES,
    'AVATAR_PROVIDER_CANDIDATE_INPUT_SIZE_INVALID',
  ...
  for (const key of ['dev', 'ino', 'size', 'mtimeMs', 'ctimeMs']) {
    assert(
      ...
      'AVATAR_PROVIDER_CANDIDATE_INPUT_CHANGED',
  ...
  try {
    ...
    fail('AVATAR_PROVIDER_CANDIDATE_UTF8_INVALID', `${label} is not valid UTF-8.`);
  }
  assert(
    text.charCodeAt(0) !== 0xfeff,
    'AVATAR_PROVIDER_CANDIDATE_BOM_FORBIDDEN',
  ...
  try {
    ...
    fail('AVATAR_PROVIDER_CANDIDATE_JSON_INVALID', `${label} is not valid JSON.`);
  }

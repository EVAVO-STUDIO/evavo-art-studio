...
export const EVA_TALK_NEUTRAL_LOCAL_PROTOCOL_VERSION = '2026-08-28.2';
...
export const EVA_TALK_NEUTRAL_CANVAS = Object.freeze({
  width: 1024,
  height: 1536,
});
...
const HEARTBEAT_FILE = /^heartbeat-[0-9]{6}\.json$/u;
...
export function stableFile(
  ...
  for (const key of ['dev', 'ino', 'size', 'mtimeMs', 'ctimeMs']) {
    assert(
      ...
      'EVA_TALK_NEUTRAL_QUEUE_FILE_CHANGED',
      label,
    );
  }
  return Object.freeze({
    absolute,
    bytes,
    sha256: sha256EvaTalkNeutralLocalQueueBytes(bytes),
  });
}
...
export function stableJson(filePath, label) {
  ...
  try {
    ...
    assert(
      ...
      'EVA_TALK_NEUTRAL_QUEUE_BOM_FORBIDDEN',
    );
    value = JSON.parse(text);
  } catch (error) {
    if (error instanceof EvaTalkNeutralLocalQueueError) throw error;
    fail(
      'EVA_TALK_NEUTRAL_QUEUE_JSON_INVALID',
...
export function writeJsonReplaceAtomic(filePath, value) {
  const parent = ensureRealDirectory(path.dirname(filePath), 'atomic JSON parent');
  const token = createHash('sha256')
    .update(`${process.pid}:${Date.now()}:${Math.random()}`)
    .digest('hex')
    .slice(0, 16);
  const temporary = path.join(parent, `.${path.basename(filePath)}.${token}.tmp`);
  try {
    writeCreateOnly(temporary, jsonBytes(value));
...
export function removeDirectoryIfPresent(directory) {
...
export function directoryChildDirectories(directory) {
  return readdirSync(directory, { withFileTypes: true })
...
export function heartbeatFilename(value) {
  assert(
    typeof value === 'string' && HEARTBEAT_FILE.test(value),
    'EVA_TALK_NEUTRAL_QUEUE_HEARTBEAT_FILE_INVALID',
  );
  return value;
}

export function chronologicalCompare(left, right) {
  return Date.parse(left) - Date.parse(right);
}

export function requireNotExpired(expiresAt, observedAt) {
  assert(
    Date.parse(observedAt) <= Date.parse(expiresAt),
    'EVA_TALK_NEUTRAL_QUEUE_CLAIM_EXPIRED',
  );
}

export function moveDirectoryAtomic(source, target) {
  assert(!existsSync(target), 'EVA_TALK_NEUTRAL_QUEUE_TARGET_ALREADY_EXISTS');

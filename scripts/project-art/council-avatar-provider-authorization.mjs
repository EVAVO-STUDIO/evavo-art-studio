...
function boundedText(value, name, maximum = 4096) {
  ...
  if (!result || result.length > maximum) throw new Error(`${name} must contain 1-${maximum} characters`);
  return result;
}
...
export async function compileCouncilAvatarProviderExecutionAuthorization({
  ...
  if (end.milliseconds <= start.milliseconds || end.milliseconds - start.milliseconds > MAXIMUM_AUTHORIZATION_DURATION_MS) {
  ...
  if (readiness.remoteProviderCallPerformed !== false || readiness.readiness.remoteCallabilityVerified !== false) {
  ...
  if (jobs.length !== 2 || jobs.some((job) => job.maximumAttempts !== 1 || job.queue !== 'provider' || job.kind !== 'art.candidate.generate')) {
  ...
  const body = Object.freeze({
    ...
    adapter: Object.freeze({ id: runtimePackage.preferredAdapterId, model: runtimePackage.preferredModel, fallbackAllowed: false }),
...
export function validateCouncilAvatarProviderExecutionAuthorization(
  ...
  if (!authorization || typeof authorization !== 'object' || authorization.schema !== COUNCIL_AVATAR_PROVIDER_EXECUTION_AUTHORIZATION_SCHEMA || authorization.status !== 'authorized') {
  ...
  for (const [index, expected] of runtimePackage.jobs.entries()) {
    ...
    if (
      ...
      throw new Error(`Council avatar provider execution authorization job ${index} drifted`);
    }
  }

...
export async function loadHeavyMetalFightingStudio(options = {}) {
  ...
  const [campaignPlan, mechanicalContract, combatPresentationContract] = await Promise.all([
  ...
  return Object.freeze({campaignRequestPath,mechanicalContractPath,combatPresentationContractPath,campaignPlan,mechanicalContract,combatPresentationContract,studioPlan});
}

export async function heavyMetalFightingSummary(options = {}) { const loaded = await loadHeavyMetalFightingStudio(options); return Object.freeze({...studioSummary(loaded.studioPlan),productionDesign:productionDesignSummary(loaded.combatPresentationContract)}); }
...
export async function heavyMetalFightingHandoffTemplate(input, options = {}) {
  const loaded = await loadHeavyMetalFightingStudio(options);
  const withoutCombinedHash = {...handoffTemplate(loaded.studioPlan,input),combatPresentationContractSha256:loaded.combatPresentationContract.contractSha256,sourceReview:loaded.combatPresentationContract.sourceReview,gameRevisionReviewStatus:input.gameRevisionSha===loaded.combatPresentationContract.sourceReview.lastReviewedGameRevision?"matches-last-reviewed-game-revision":"requires-fresh-live-game-source-review"};
...
export async function verifyHeavyMetalFightingStudio(options = {}) {
  ...
  const checks = Object.freeze([...studioVerification.checks,...productionVerification.checks.map((check)=>Object.freeze({id:`production-${check.id}`,passed:check.passed}))]);
  const failed = Object.freeze(checks.filter((check)=>!check.passed));
  return Object.freeze({schema:"evavo.heavy-metal-fighting-combined-verification.v1",status:failed.length?"failed":"passed",studioPlanSha256:loaded.studioPlan.studioPlanSha256,combatPresentationContractSha256:loaded.combatPresentationContract.contractSha256,productionVerificationSha256:productionVerification.verificationSha256,checks,failed,warnings:loaded.studioPlan.warnings});
}

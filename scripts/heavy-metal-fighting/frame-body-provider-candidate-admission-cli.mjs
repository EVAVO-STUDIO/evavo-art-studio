...
function usage() {
  return [
    ...
    "",
    "plan validates the immutable runtime chain, artifact descriptors, evidence and exact 160x160 RGBA PNG without writing.",
...
async function receipts(filePath) {
  ...
  throw new Error("--receipts-json must contain a receipt array or a receipt bundle with a receipts array.");
}
async function inputs(argv) {
  return {
    dispatch: await json(option(argv, "--dispatch-json"), "--dispatch-json"),
    binding: await json(option(argv, "--runtime-binding-json"), "--runtime-binding-json"),
    outcome: await json(option(argv, "--runtime-outcome-json"), "--runtime-outcome-json"),
    options: {
...
async function run(argv = process.argv.slice(2)) {
  ...
  if (command === "plan") {
    const input = await inputs(argv.slice(1));
    return planHmfProviderCandidateAdmission(input.dispatch, input.binding, input.outcome, input.options);
  }
  if (command === "admit") {
    const input = await inputs(argv.slice(1));
    return admitHmfProviderCandidate(input.dispatch, input.binding, input.outcome, {
      ...input.options,
      workspaceRoot: option(argv.slice(1), "--workspace-root"),
      writeEnabled: argv.includes("--write"),
    });
  }

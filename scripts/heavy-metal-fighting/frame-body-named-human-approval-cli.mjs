...
function usage() {
  return [
    ...
    "  node scripts/heavy-metal-fighting/frame-body-named-human-approval-cli.mjs plan --mastering-plan-json <file> --workspace-root <root> --human-approval-json <file>",
    "  node scripts/heavy-metal-fighting/frame-body-named-human-approval-cli.mjs materialize --approval-plan-json <file>",
    "",
    "plan is read-only. materialize persists one immutable named-human approval record and appends exactly one named-human-approved receipt. It does not mutate the master, promote it into the game repository, compile delivery readiness or the final atlas, commit, push, deploy or publish.",
  ].join("\n");
}
...
async function run(argv = process.argv.slice(2)) {
  ...
  if (command === "plan") {
    ...
    return compileHmfFrameBodyNamedHumanApprovalPlan({
  ...
  if (command === "materialize") {
    return materializeHmfFrameBodyNamedHumanApproval(
      await json(option(argv, "--approval-plan-json"), "--approval-plan-json"),
    );
  }

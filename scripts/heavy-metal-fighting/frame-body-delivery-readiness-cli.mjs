...
function usage() {
  return [
    ...
    "",
    "plan is read-only. materialize persists one immutable delivery-readiness record and appends exactly one delivery-ready receipt. It does not copy the master to a game repository, compile the final atlas, mutate a target repository, commit, push, deploy or publish.",
  ].join("\n");
}
...
async function run(argv = process.argv.slice(2)) {
  ...
  if (command === "plan") {
    ...
    return compileHmfFrameBodyDeliveryReadinessPlan({
      approvalPlan: await json(option(argv, "--approval-plan-json"), "--approval-plan-json"),
      workspaceRoot,
      readinessRequest: await json(option(argv, "--readiness-request-json"), "--readiness-request-json"),
    });
  }

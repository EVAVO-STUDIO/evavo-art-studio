...
async function readStablePolicy() {
  ...
  assert(
    ...
    "delivery-readiness policy must be a regular non-symlink file.",
  );
  ...
  try {
    ...
    fail(
      `delivery-readiness policy is invalid UTF-8 JSON: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
...
function validatePolicy(raw) {
  ...
  assert(
    ...
    "delivery-readiness next legal action drifted.",
  );
  const authority = raw.authority ?? {};
  assert(
    authority.masterRead === true
      && authority.approvalRecordRead === true
      && authority.deliveryReadinessRecordPersistence === true
      && authority.receiptPersistence === true
      && authority.deliveryReadinessCompilation === true,
    "delivery-readiness policy lost its bounded workspace authority.",
  );

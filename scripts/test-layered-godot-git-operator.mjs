...
async function fixture() {
  ...
  const remote = await mkdtemp(
    path.join(os.tmpdir(), "evavo-git-operator-remote-"),
  );
  ...
  return {
    ...
    realRoot,
    resources: entries,
    baselineHead: git(root, "rev-parse", "HEAD"),
  };
}
...
function dependencies(fx, currentReview, options = {}) {
  return {
    ...
    inspectWorkspaceRoot:
      options.inspectWorkspaceRoot ??
      (async () => ({ path: fx.root, realPath: fx.realRoot })),
    sameFilesystemPath: (left, right) =>
      path.resolve(left) === path.resolve(right),
    verifyWriteRequest:
      options.verifyWriteRequest ??
      (() => ({
        requestSha256: REQUEST_SHA,
        integration: {
          integrationSha256: INTEGRATION_SHA,
    ...
    reviewRepository:
      ...
      (async () => {
        if (options.beforeReviewReturn) {
          await options.beforeReviewReturn(fx);
        }
        return currentReview;
      }),
    readStableRegularFile: async (filePath) => {
      if (options.beforeStableRead) {
        await options.beforeStableRead(filePath, fx);
      }
    ...
    runGit:
      options.runGit ??
      ((root, args, runOptions) => runGit(root, args, runOptions)),
  };
}
...
async function expectCode(promise, code) {
  await assert.rejects(
    promise,
    (error) =>
      error instanceof LayeredGodotGitOperatorError && error.code === code,
  );
}
...
test(
  "creates one exact commit for seven reviewed untracked resources without pushing or hooks",
  async (t) => {
    ...
    const result = await commitLayeredGodotHandoff(
      operatorInput(fx, review),
      dependencies(
        fx,
        makeReview(fx, { reviewedAt: "2026-08-13T00:00:01.000Z" }),
      ),
    );
    ...
    assert.equal(
      git(fx.remote, "rev-parse", "refs/heads/main"),
      remoteBefore,
    );
    ...
    for (const resource of fx.resources) {
      assert.deepEqual(
        gitBuffer(fx.root, "show", `HEAD:${resource.path}`),
        resource.data,
      );
    }
  },
...
test(
  "commits an exact mix of tracked modifications and new reviewed resources",
  async (t) => {
    const fx = await fixture();
    await cleanup(t, fx);
    await writeResources(fx.root, fx.resources);
    ...
    fx.resources[1] = {
      ...fx.resources[1],
      content: changedData.toString(),
      data: changedData,
      sha256: sha(changedData),
      bytes: changedData.byteLength,
    };
    const review = makeReview(fx, {
      modified: [RESOURCE_PATHS[1]],
      untracked: RESOURCE_PATHS.slice(2),
      unchanged: [RESOURCE_PATHS[0]],
    });
    const current = makeReview(fx, {
      modified: [RESOURCE_PATHS[1]],
      untracked: RESOURCE_PATHS.slice(2),
      unchanged: [RESOURCE_PATHS[0]],
      reviewedAt: "2026-08-13T00:00:02.000Z",
    });
    const result = await commitLayeredGodotHandoff(
      operatorInput(fx, review),
      dependencies(fx, current),
    );
    assert.equal(result.commit.parent, fx.baselineHead);
    assert.deepEqual(
      git(
        ...
        .filter(Boolean)
        .sort(),
      [RESOURCE_PATHS[1], ...RESOURCE_PATHS.slice(2)].sort(),
    );
  },
);

test("returns a no-op receipt when all seven resources are integrated", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  ...
  const result = await commitLayeredGodotHandoff(
    ...
    dependencies(
      ...
      makeReview(fx, {
        untracked: [],
        unchanged: RESOURCE_PATHS,
        reviewedAt: "2026-08-13T00:00:03.000Z",
      }),
    ),
  );
...
test("rejects push or force-push authority before touching Git", async (t) => {
  ...
  await expectCode(
    commitLayeredGodotHandoff(
      operatorInput(fx, review, {
        authorization: { commit: true, push: true, forcePush: false },
      }),
      dependencies(fx, review),
    ),
    "LAYERED_GODOT_GIT_OPERATOR_AUTHORITY_INVALID",
  );
  await expectCode(
    commitLayeredGodotHandoff(
      operatorInput(fx, review, {
        authorization: { commit: true, push: false, forcePush: true },
      }),
      dependencies(fx, review),
    ),
    "LAYERED_GODOT_GIT_OPERATOR_AUTHORITY_INVALID",
...
test("rejects multiline commit messages before Git mutation", async (t) => {
  ...
  await expectCode(
    commitLayeredGodotHandoff(
      operatorInput(fx, review, { commitMessage: "bad\nmessage" }),
      dependencies(fx, review),
    ),
    "LAYERED_GODOT_GIT_OPERATOR_INPUT_INVALID",
  );
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
});

test("rejects a rehashed review that escalates commit authority", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const bad = makeReview(fx, {
    overrides: {
      readiness: { ...makeReview(fx).readiness, gitCommitAuthorized: true },
    },
  });
  await expectCode(
    commitLayeredGodotHandoff(
      operatorInput(fx, bad),
      dependencies(fx, bad),
    ),
    "LAYERED_GODOT_GIT_OPERATOR_REVIEW_INVALID",
  );
});
...
test("rejects repository-review semantic drift before staging", async (t) => {
  ...
  const review = makeReview(fx);
  const current = makeReview(fx, {
    untracked: RESOURCE_PATHS.slice(1),
    unchanged: [RESOURCE_PATHS[0]],
    reviewedAt: "2026-08-13T00:00:04.000Z",
  });
  await expectCode(
    commitLayeredGodotHandoff(
      operatorInput(fx, review),
    ...
    "LAYERED_GODOT_GIT_OPERATOR_REVIEW_DRIFT",
  );
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
});

test("detects worktree drift after staging and restores only the index", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  ...
  const deps = dependencies(
    fx,
    makeReview(fx, { reviewedAt: "2026-08-13T00:00:05.000Z" }),
    {
  ...
  await expectCode(
    ...
    "LAYERED_GODOT_GIT_OPERATOR_WORKTREE_DRIFT",
  );
  assert.equal(git(fx.root, "rev-parse", "HEAD"), fx.baselineHead);
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
  assert.equal(
    await readFile(
      path.join(fx.root, ...RESOURCE_PATHS[0].split("/")),
      "utf8",
    ),
    "external drift\n",
  );
...
test("stages raw bytes without invoking an active clean filter", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  ...
  await expectCode(
    commitLayeredGodotHandoff(
      ...
      dependencies(
        fx,
        makeReview(fx, { reviewedAt: "2026-08-13T00:00:06.000Z" }),
      ),
    ),
    "LAYERED_GODOT_GIT_OPERATOR_GIT_TRANSFORM_ACTIVE",
  );
  await assert.rejects(access(sentinel));
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
});

test("rolls back staged index when commit identity is unavailable", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  ...
  await expectCode(
    commitLayeredGodotHandoff(
      ...
      dependencies(
        fx,
        makeReview(fx, { reviewedAt: "2026-08-13T00:00:07.000Z" }),
      ),
    ),
    "LAYERED_GODOT_GIT_OPERATOR_COMMIT_FAILED",
  );
  assert.equal(git(fx.root, "rev-parse", "HEAD"), fx.baselineHead);
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
});

test("detects HEAD drift and rolls the staged index back", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  let firstRun = true;
  const baseRun = (root, args, options) => runGit(root, args, options);
  const deps = dependencies(
    fx,
    makeReview(fx, { reviewedAt: "2026-08-13T00:00:08.000Z" }),
    {
      runGit: async (root, args, options) => {
        ...
        return baseRun(root, args, options);
      },
    },
  );
  await expectCode(
    commitLayeredGodotHandoff(operatorInput(fx, review), deps),
    "LAYERED_GODOT_GIT_OPERATOR_REPOSITORY_DRIFT",
  );
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
});
...
test("input snapshot rejects an accessor without invoking it", async (t) => {
  ...
  Object.defineProperty(input, "commitMessage", {
    enumerable: true,
    get() {
      invoked = true;
      return "bad";
  ...
  await expectCode(
    commitLayeredGodotHandoff(input, dependencies(fx, review)),
    "LAYERED_GODOT_GIT_OPERATOR_INPUT_INVALID",
  );
...
test("Git runner fails closed on output limits", async (t) => {
  ...
  await expectCode(
    runGit(fx.root, ["--version"], {
      gitExecutable: fake,
      maximumBytes: 128,
    }),
    "LAYERED_GODOT_GIT_OPERATOR_GIT_OUTPUT_LIMIT",
  );
});

test("Git runner fails closed on timeout", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  ...
  await expectCode(
    ...
    "LAYERED_GODOT_GIT_OPERATOR_GIT_TIMEOUT",
  );
});

test("refuses rollback over a concurrent same-path index change", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  ...
  const deps = dependencies(
    fx,
    makeReview(fx, { reviewedAt: "2026-08-13T00:00:09.000Z" }),
    {
      runGit: async (root, args, options) => {
        if (!injected && args[0] === "check-attr") {
          ...
          git(
            ...
            "--add",
            "--cacheinfo",
            `100644,${externalOid},${RESOURCE_PATHS[0]}`,
          );
  ...
  assert.deepEqual(
    gitBuffer(fx.root, "show", `:${RESOURCE_PATHS[0]}`),
    externalData,
  );
  assert.deepEqual(
    await readFile(path.join(fx.root, ...RESOURCE_PATHS[0].split("/"))),
    fx.resources[0].data,
  );
});

test("preserves the authorized single-line commit message verbatim", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  ...
  const result = await commitLayeredGodotHandoff(
    ...
    dependencies(
      fx,
      makeReview(fx, { reviewedAt: "2026-08-13T00:00:10.000Z" }),
    ),
  );
...
test("dependency capture rejects an accessor without invoking it", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  const deps = dependencies(fx, review);
  let invoked = false;
...
test("dependency capture rejects unsupported fields and Proxy functions", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  await expectCode(
    commitLayeredGodotHandoff(operatorInput(fx, review), {
      ...dependencies(fx, review),
      surpriseMutationAuthority: true,
    }),
    "LAYERED_GODOT_GIT_OPERATOR_INPUT_INVALID",
  );
  const proxied = dependencies(fx, review);
  proxied.runGit = new Proxy(proxied.runGit, {});
  await expectCode(
    commitLayeredGodotHandoff(operatorInput(fx, review), proxied),
    "LAYERED_GODOT_GIT_OPERATOR_INPUT_INVALID",
  );
  assert.equal(git(fx.root, "diff", "--cached", "--name-only"), "");
});

test("dependency mutation after the API call cannot replace captured functions", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  ...
  const deps = dependencies(fx, makeReview(fx, {
    reviewedAt: "2026-08-13T00:00:11.000Z",
  }), {
    inspectWorkspaceRoot: async () => {
      await inspectionGate;
      return { path: fx.root, realPath: fx.realRoot };
    },
  });
  const operation = commitLayeredGodotHandoff(operatorInput(fx, review), deps);
  deps.verifyWriteRequest = () => {
    throw new Error("late dependency replacement executed");
  };
  deps.runGit = async () => {
    throw new Error("late Git dependency replacement executed");
  };
  releaseInspection();
...
test("workspace-root result accessors are rejected without invocation", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  const review = makeReview(fx);
  let invoked = false;
  const deps = dependencies(fx, review, {
    inspectWorkspaceRoot: async () => {
      const result = { realPath: fx.realRoot };
...
test("verifier accessors are rejected without invocation or Git mutation", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  ...
  const deps = dependencies(fx, review, {
    verifyWriteRequest: () => {
      ...
      Object.defineProperty(verified, "requestSha256", {
        enumerable: true,
        get() {
          invoked = true;
          return REQUEST_SHA;
...
test("verifier-owned resource buffers are copied before asynchronous review", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const original = Buffer.from(fx.resources[0].data);
  const review = makeReview(fx);
  const current = makeReview(fx, {
    reviewedAt: "2026-08-13T00:00:12.000Z",
  });
  const deps = dependencies(fx, current, {
    reviewRepository: async () => {
      fx.resources[0].data.fill(0x78);
      return current;
    },
  });
  const result = await commitLayeredGodotHandoff(
  ...
  assert.equal(result.outcome, "committed");
  assert.deepEqual(
    gitBuffer(fx.root, "show", `HEAD:${RESOURCE_PATHS[0]}`),
    original,
  );
});

test("verifier byte metadata mismatch is rejected before Git mutation", async (t) => {
  const fx = await fixture();
  await cleanup(t, fx);
  await writeResources(fx.root, fx.resources);
  const review = makeReview(fx);
  const malformed = fx.resources.map((entry) => ({ ...entry }));
  malformed[0].bytes += 1;
  const deps = dependencies(fx, review, {
    verifyWriteRequest: () => ({
      requestSha256: REQUEST_SHA,

...
import {
  ...
  heavyMetalFightingSpriteBank,
  heavyMetalFightingSpriteCensus,
...
const COMMANDS = Object.freeze(["verify","summary","contract","presentation-contract","sprite-census","sprite-bank","pilot","frame","moves","move","cel","slot","screen","super","intro","attract","readiness","assets","batch","style-proof","handoff-template"]);
function usage() {
  return [
    ...
    "  node scripts/heavy-metal-fighting-art-studio.mjs sprite-bank <bank-id>",
    ...
    "The current 120-cell commands remain compatibility/runtime inspection. sprite-census and sprite-bank expose the planned 160x160 / 256-slot production-master-v3 target.",
...
async function run(argv = process.argv.slice(2)) {
  ...
  if (command === "sprite-census") return heavyMetalFightingSpriteCensus();
  if (command === "sprite-bank") { if (args.length !== 1) throw new Error(`sprite-bank requires one production-master bank id.\n\n${usage()}`); return heavyMetalFightingSpriteBank(args[0]); }
  ...
  if (command === "cel") { if (args.length !== 2 || !/^\d+$/.test(args[1])) throw new Error(`cel requires a Frame id and integer compatibility source index.\n\n${usage()}`); return heavyMetalFightingSourceCel(args[0], Number(args[1])); }
  if (command === "slot") { if (args.length !== 3 || !/^\d+$/.test(args[2])) throw new Error(`slot requires a Frame id, current or planned-v2, and an integer compatibility slot.\n\n${usage()}`); return heavyMetalFightingRuntimeSlot(args[0], args[1], Number(args[2])); }
  ...
  if (command === "batch") { if (args.length !== 1 || !/^\d+$/.test(args[0])) throw new Error(`batch requires one integer compatibility-campaign batch number.\n\n${usage()}`); return heavyMetalFightingBatch(Number(args[0])); }

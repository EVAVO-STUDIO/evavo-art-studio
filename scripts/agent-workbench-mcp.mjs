...
const tools = Object.freeze([
  {
    name: "evavo_agent_workbench_capabilities",
    description: "Describe the EVAVO Agent Workbench contracts, admitted evidence roots and truth boundaries. Performs no repository or provider mutation.",
...
function contractBundle() {
  const bundlePath = path.join(repositoryRoot, ".evavo", "agent-workbench.contracts.v1.json");
  const bytes = fs.readFileSync(bundlePath);
  return { bundle: JSON.parse(bytes.toString("utf8")), evidence: { path: bundlePath, sha256: digest(bytes), bytes: bytes.length }, allowedEvidenceRoots: allowedRoots.map((item) => item.lexical), policy: { readOnly: true, executionGranted: false, mutationGranted: false, publicationGranted: false } };
}
...
for await (const line of input) {
  ...
  try {
    ...
    if (request.method === "initialize") {
      response(request.id, { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "evavo-agent-workbench", version: "1.0.0" } });
    } else if (request.method === "notifications/initialized") {
      // Notification: no response.
    } else if (request.method === "tools/list") {
      response(request.id, { tools });
    } else if (request.method === "tools/call") {
      const output = callTool(request.params?.name, request.params?.arguments ?? {});
      response(request.id, { content: [{ type: "text", text: JSON.stringify(output, null, 2) }], structuredContent: output, isError: false });
    } else {

...
const ORIGINS = new Set(["explicit", "repository-config", "sibling-development-studio", "unavailable"]);
...
export function verifyAgentWorkbenchAwareness(snapshotRead) {
  ...
  assert(ORIGINS.has(discovery.toolRegistryOrigin), "discovery.toolRegistryOrigin is invalid.");
...
function selfTest() {
  ...
  const snapshot = {
    ...
    matches: { tools: [{ id: "mcp:test", source: "root-mcp-launch-manifest", runtimeReadiness: "unknown", commandBaseName: "node", argumentCount: 1, environmentKeys: ["MODE"] }] },
  ...
  unsafe.matches.tools[0].args = ["secret"];
  let rejected = false;
  try { verifyAgentWorkbenchAwareness({ value: unsafe }); } catch { rejected = true; }
  assert(rejected, "Awareness verifier must reject raw MCP args.");
  process.stdout.write(`${JSON.stringify({ contract: "evavo_agent_workbench_awareness_verifier_self_test_v1", status: "passed", assertions: 3 }, null, 2)}\n`);
}

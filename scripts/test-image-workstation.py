...
class ImageWorkstationTests(unittest.TestCase):
    ...
    def test_source_hash(self):
        plan={'schema':'evavo.raster-workstation-plan.v1','input':'in.png','output':'out.png','sourceSha256':'0'*64,'createOnlyOutput':True,'sourceOverwrite':False,'operations':[]}; (self.r/'plan.json').write_text(json.dumps(plan)); p=subprocess.run(['python',str(SCRIPT),'--workspace-root',str(self.r),'--plan',str(self.r/'plan.json'),'--receipt',str(self.r/'receipt.json')],capture_output=True,text=True); self.assertEqual(p.returncode,2); self.assertIn('source hash mismatch',p.stderr)

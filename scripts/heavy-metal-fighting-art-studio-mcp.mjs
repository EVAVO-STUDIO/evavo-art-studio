...
export function toolDefinitions() {
  return Object.freeze([
    {name:SUMMARY_TOOL,description:"Return the exact launch-four inventory, hashes, Frames, production-design summary, atlas status, style-proof blockers and authority boundary.",inputSchema:objectSchema()},
    ...
    {name:PRESENTATION_CONTRACT_TOOL,description:"Return the normalized combat, move, screen, HUD, super, intro and exact 1,157-image allocation contract.",inputSchema:objectSchema()},
    ...
    {name:FRAME_TOOL,description:"Return one complete Frame source-cel plan with 120 authored cels and current/planned runtime-slot maps.",inputSchema:objectSchema({frameId:{type:"string",enum:FRAME_IDS}},["frameId"])},
    ...
    {name:MOVE_TOOL,description:"Return one move production plan with live timing where implemented, nine-cel choreography, bank mappings, effects and gates.",inputSchema:objectSchema({frameId:{type:"string",enum:FRAME_IDS},moveId:{type:"string",minLength:1}},["frameId","moveId"])},
    {name:SOURCE_CEL_TOOL,description:"Return one exact authored Frame source cel with mechanical identity, neighbours, runtime bindings and review gates.",inputSchema:objectSchema({frameId:{type:"string",enum:FRAME_IDS},sourceIndex:{type:"integer",minimum:0,maximum:119}},["frameId","sourceIndex"])},
    {name:RUNTIME_SLOT_TOOL,description:"Inspect one current or planned-v2 runtime atlas slot, collision/reserved state and source bindings.",inputSchema:objectSchema({frameId:{type:"string",enum:FRAME_IDS},mapName:{type:"string",enum:["current","planned-v2"]},slot:{type:"integer",minimum:0,maximum:119}},["frameId","mapName","slot"])},
    ...
    {name:ASSET_TOOL,description:"Return the full 1,157-image allocation or one exact family allocation.",inputSchema:objectSchema({familyId:{type:"string",enum:FAMILY_IDS}})},
    {name:BATCH_TOOL,description:"Return one exact family-locked production batch with up to ten separate image work units.",inputSchema:objectSchema({batchNumber:{type:"integer",minimum:1,maximum:119}},["batchNumber"])},
    ...
    {name:VERIFY_TOOL,description:"Run deterministic campaign, mechanical, move, UI, super, intro, allocation, authority and style-proof checks without writes.",inputSchema:objectSchema()},
    {name:HANDOFF_TOOL,description:"Compile a read-only handoff template bound to one exact game commit, live slot-manifest hash and presentation contract hash.",inputSchema:objectSchema({gameRevisionSha:{type:"string",pattern:"^[0-9a-f]{40}$"},liveSlotManifestSha256:{type:"string",pattern:"^[0-9a-f]{64}$"}},["gameRevisionSha","liveSlotManifestSha256"])},
  ]);
}

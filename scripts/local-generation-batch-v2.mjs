...
export function imageMetadata(plan, frame, { attempt, route = null, candidate = null, qa = null } = {}) {
  return Object.freeze({
    ...
    settings: plan.provider?.backend === 'draw-things'
      ? {
          width: frame.shot.target?.width ?? plan.quality.width,
          height: frame.shot.target?.height ?? plan.quality.height,
          outputFormat:
            frame.shot.target?.outputFormat ?? plan.quality.outputFormat,
          candidateCount: frame.candidateCount,
          samplingAuthority: 'governed-draw-things-profile',
          executedSamplingSettingsInProviderEvidence: true,
        }
      : {
          width: frame.shot.target?.width ?? plan.quality.width,
          height: frame.shot.target?.height ?? plan.quality.height,
          ...
          outputFormat:
            frame.shot.target?.outputFormat ?? plan.quality.outputFormat,
          candidateCount: frame.candidateCount,
          samplingAuthority: 'managed-comfyui-quality-profile',
          executedSamplingSettingsInProviderEvidence: true,
        },

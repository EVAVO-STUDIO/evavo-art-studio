...
export const SPEC_SCHEMA = "evavo.rally-skid-smoke-visual-development.v1";
export const SESSION_SCHEMA = "evavo.rally-skid-smoke-provider-job-session.v1";
export const JOB_SCHEMA = "evavo.rally-skid-smoke-provider-job.v1";
export const PROTOCOL_VERSION = "2026-08-15.7";
...
const CONTINUITY_KEYS = new Set([
  "rearWheelContactOrigin", "travelDirection", "windResponseReadable",
  "consistentSmokeTemperature", "consistentOpacityHierarchy", "roadBackgroundExcluded",
]);
...
const REQUIRED_ROLES = new Set([
  "effect-shape-master", "effect-response-reference",
  "particle-sprite-reference", "decay-reference",
]);

function fail(message) {
  throw new Error(`RALLY_SKID_SMOKE_ART_JOBS_INVALID: ${message}`);
}
...
function validateContinuity(value) {
  ...
  for (const key of [
    "rearWheelContactOrigin", "windResponseReadable", "consistentSmokeTemperature",
    "consistentOpacityHierarchy", "roadBackgroundExcluded",
  ]) {
    if (value[key] !== true) fail(`continuity.${key} must remain true.`);
  }
  if (value.travelDirection !== "vehicle-rearward") {
...
function validateOutput(output) {
  ...
  if (
    output.oneImagePerJob !== true || output.retainIndividualSources !== true ||
...
export function validateSkidSmokeSpec(input) {
  ...
  if (!Array.isArray(source.jobs) || source.jobs.length !== 10) {
    fail("jobs must contain exactly ten one-image skid-smoke jobs.");
  }
  const ids = new Set();
  const jobs = source.jobs.map((raw, index) => {
    ...
    if (raw.transparent !== true) fail(`jobs[${index}] must request true transparency.`);
    const dependsOn = stringArray(raw.dependsOn, `jobs[${index}].dependsOn`).map((entry) => identifier(entry, `jobs[${index}].dependsOn`));
    const include = stringArray(raw.include, `jobs[${index}].include`, 3, 20);
    const exclude = stringArray(raw.exclude, `jobs[${index}].exclude`, 3, 20);
  ...
  if (jobs.filter((job) => job.role === "effect-response-reference").length !== 5) {
    fail("skid smoke requires exactly five response-reference jobs.");
  }
  if (jobs.filter((job) => job.role === "particle-sprite-reference").length !== 3) {
    fail("skid smoke requires exactly three particle-sprite jobs.");
  }
  return source;
}
...
function compilePrompt(spec, job) {
  return [
    ...
    `Projection: ${spec.style.projectionFamily}. Preserve rear-wheel contact origin, rearward travel, smoke temperature, opacity hierarchy and wind response established by dependencies.`,
    `Include: ${job.include.join("; ")}.`,
    `Avoid: ${[...spec.style.forbidden, ...job.exclude].join("; ")}.`,
    `Output one ${job.width}x${job.height} PNG with true transparent background. No labels, no contact sheet, no grid and no second effect.`,
  ].join("\n");
}

export function compileSkidSmokeProviderSession(input) {
  const spec = validateSkidSmokeSpec(input);
  ...
  const body = {
    ...
    totals: {
      ...
      decayJobs: jobs.filter((job) => job.role === "decay-reference").length,
...
export function verifySkidSmokeProviderSession(session) {
  ...
  if (sha256(body) !== session.sessionSha256) {
    fail("sessionSha256 does not match the submitted payload.");
  }
  const expected = compileSkidSmokeProviderSession(session.spec);
...
async function main() {
  ...
  if (!["compile", "verify"].includes(command) || !inputPath) {
    fail("Usage: rally-25d-skid-smoke-jobs.mjs <compile|verify> <input.json> [--output output.json]");
  }
  const input = JSON.parse(await readFile(inputPath, "utf8"));
  const result = command === "compile"
    ? compileSkidSmokeProviderSession(input)
    : {
        valid: verifySkidSmokeProviderSession(input),

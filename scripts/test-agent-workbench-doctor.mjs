...
for (const repoRoot of [root, sibling]) {
  fs.mkdirSync(path.join(repoRoot, ".evavo"), { recursive: true });
...
if (result.sharedDrift.status !== "aligned" || result.sharedDrift.comparedRepositoryCount !== 1 || result.sharedDrift.mismatchCount !== 0) throw new Error("doctor did not prove aligned shared files");
...
if (truthBoundary.sharedDriftCheckReadOnly !== true || truthBoundary.sharedDriftAutomaticRepair !== false || truthBoundary.sharedDriftUnavailableDoesNotProveAlignment !== true) throw new Error("fixture shared drift truth boundary mismatch");
...
if (result.artifactCount !== 26) throw new Error(`unexpected artifact count ${result.artifactCount}`);
if (!result.artifacts.some((item) => item.path === orientationGuide)) throw new Error("orientation guide was not health checked");

fs.writeFileSync(path.join(sibling, "scripts", "verify-agent-workbench-guidance.mjs"), "//intentional-drift\n");
result = inspectAgentWorkbench(root, { workspaceRoot: workspace });
if (result.status !== "repair-required" || result.registration.state !== "registered") throw new Error("shared drift did not fail health independently of registration");
if (result.sharedDrift.status !== "drifted" || result.sharedDrift.mismatchCount !== 1) throw new Error("shared drift evidence mismatch");
...
console.log(JSON.stringify({ contract:"evavo_agent_workbench_doctor_test_v8", status:"passed", assertions:17 }, null, 2));

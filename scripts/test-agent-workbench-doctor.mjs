...
fs.writeFileSync(path.join(root, ".evavo", "agent-workbench.mcp.v2.json"), JSON.stringify({ contractVersion: "evavo_agent_workbench_mcp_v2", repository, authority, readOnly: true, tools: ["evavo_agent_workbench_capabilities", "evavo_agent_workbench_snapshot", "evavo_agent_workbench_guide", "evavo_agent_workbench_handoff"], truthBoundary: { grantsExecutionAuthority: false, grantsMutationAuthority: false, grantsPublicationAuthority: false, guidanceProvesReadiness: false } }));
const entrypoints = { compileSnapshot:"scripts/a", verify:"scripts/b", compileHandoff:"scripts/c", guide:"scripts/d", guideTest:"scripts/e", verifyGuidance:"scripts/f", mcp:"scripts/g", mcpSmoke:"scripts/h", mcpV2:"scripts/agent-workbench-mcp-v2.mjs", mcpV2Smoke:"scripts/j" };
const schemas = { config:"schemas/a", snapshot:"schemas/b", handoff:"schemas/c", guidance:"schemas/d" };
fs.writeFileSync(path.join(root, ".evavo", "agent-workbench.contracts.v1.json"), JSON.stringify({ contractVersion:"evavo_agent_workbench_contract_bundle_v1", repository, authority, mcpV2:".evavo/agent-workbench.mcp.v2.json", entrypoints, schemas }));
...
if (result.artifactCount !== 16) throw new Error(`unexpected artifact count ${result.artifactCount}`);
console.log(JSON.stringify({ contract:"evavo_agent_workbench_doctor_test_v1", status:"passed", assertions:5 }, null, 2));

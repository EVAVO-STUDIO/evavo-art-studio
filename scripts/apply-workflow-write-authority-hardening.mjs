...
async function rewritePublisher({
  ...
  after = replaceExact(
    ...
    newSteps,
    1,
    `${workflowPath} authorization step`,
  );
...
const finalizerWriteBefore = `  [\n    ".github/workflows/finalize-pixel-typography-review.yml",\n    {\n      events: ["push", "workflow_dispatch"],\n      writes: ["contents"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "git push origin HEAD:main",\n        'test "$remote" = "$published"',\n        "cancel-in-progress: false",\n      ],\n    },\n  ],`;
const finalizerWriteAfter = `  [\n    ".github/workflows/finalize-pixel-typography-review.yml",\n    {\n      events: ["workflow_dispatch"],\n      writes: ["contents"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "expected_sha:",\n        "FINALIZE_PIXEL_TYPOGRAPHY_REVIEW",\n        "git push origin HEAD:main",\n        'test "$remote" = "$published"',\n        "cancel-in-progress: false",\n      ],\n    },\n  ],`;

testSource = replaceExact(
  testSource,
  finalizerWriteBefore,
  finalizerWriteAfter,
  1,
  "finalizer write allowlist",
);

const reconciliationWriteBefore = `  [\n    ".github/workflows/repair-pixel-typography-review.yml",\n    {\n      events: ["workflow_dispatch"],\n      writes: ["contents"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "git push origin HEAD:main",\n        'test "$remote" = "$published"',\n        "cancel-in-progress: false",\n      ],\n    },\n  ],`;
const reconciliationWriteAfter = `  [\n    ".github/workflows/repair-pixel-typography-review.yml",\n    {\n      events: ["workflow_dispatch"],\n      writes: ["contents"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "expected_sha:",\n        "RECONCILE_PIXEL_TYPOGRAPHY_REVIEW",\n        "git push origin HEAD:main",\n        'test "$remote" = "$published"',\n        "cancel-in-progress: false",\n      ],\n    },\n  ],`;

testSource = replaceExact(
  testSource,
  reconciliationWriteBefore,
  reconciliationWriteAfter,
  1,
  "reconciliation write allowlist",
);

const finalizerPrivilegedBefore = `  [\n    ".github/workflows/finalize-pixel-typography-review.yml",\n    {\n      events: ["push", "workflow_dispatch"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "git push origin HEAD:main",\n        "Require exact current main and reviewed baseline files",\n      ],\n    },\n  ],`;
const finalizerPrivilegedAfter = `  [\n    ".github/workflows/finalize-pixel-typography-review.yml",\n    {\n      events: ["workflow_dispatch"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "expected_sha:",\n        "FINALIZE_PIXEL_TYPOGRAPHY_REVIEW",\n        "git push origin HEAD:main",\n        "Require explicit manual authorization",\n        "Require exact current main and reviewed baseline files",\n      ],\n    },\n  ],`;

testSource = replaceExact(
  testSource,
  finalizerPrivilegedBefore,
  finalizerPrivilegedAfter,
  1,
  "finalizer privileged allowlist",
);

const reconciliationPrivilegedBefore = `  [\n    ".github/workflows/repair-pixel-typography-review.yml",\n    {\n      events: ["workflow_dispatch"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "git push origin HEAD:main",\n        "Require explicit publication confirmation and token",\n      ],\n    },\n  ],`;
const reconciliationPrivilegedAfter = `  [\n    ".github/workflows/repair-pixel-typography-review.yml",\n    {\n      events: ["workflow_dispatch"],\n      requiredEvidence: [\n        "permissions:\\n  contents: write",\n        "persist-credentials: true",\n        "expected_sha:",\n        "RECONCILE_PIXEL_TYPOGRAPHY_REVIEW",\n        "git push origin HEAD:main",\n        "Require explicit manual authorization",\n        "Publish only the governed reconciliation",\n      ],\n    },\n  ],`;

testSource = replaceExact(
  testSource,
  reconciliationPrivilegedBefore,
  reconciliationPrivilegedAfter,
  1,
  "reconciliation privileged allowlist",
);
...
assert.equal(
  testSource.includes(
    'test("repository write workflows are manual-only and SHA-confirmed"',
  ),
  false,
);

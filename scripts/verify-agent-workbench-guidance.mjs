...
export function verifyAgentWorkbenchGuidance(guidanceRead, snapshotRead) {
  ...
  if (guide.disposition === "mcp-runtime-verification-required") {
    assert(selected?.type === "tool", "MCP runtime guidance requires a selected tool route.");
    assert(selected.evidence?.source === "root-mcp-launch-manifest", "MCP runtime guidance requires root MCP launch evidence.");
    assert(selected.evidence?.runtimeReadiness === "unknown", "MCP runtime guidance must retain runtimeReadiness=unknown.");
    assert(guide.blockers.some((item) => typeof item === "string" && item.includes("live protocol/discovery receipt")), "MCP runtime guidance must require live protocol/discovery evidence.");
  }
...
function selfTest() {
  ...
  const mcpRoute = { id: "mcp:test", source: "root-mcp-launch-manifest", repository: "EVAVO-STUDIO/test", runtimeReadiness: "unknown", relevance: 1 };
  const snapshot = { contract: "evavo_agent_workbench_snapshot_v1", repository: "EVAVO-STUDIO/test", matches: { capabilities: [route], scripts: [], tools: [mcpRoute], estateCapabilities: [] }, policy: { readOnlyCompiler: true, commandExecutionPerformed: false, mutationPerformed: false, routeDoesNotAuthorizeEffects: true, planDoesNotProveExecution: true, publicationRequiresSeparateAuthority: true } };
  ...
  const guidance = { ...base, selectedRoute: { type: "capability", id: "test.read", selection: "explicit", relevance: 2, sha256: digest(Buffer.from(JSON.stringify(route), "utf8")), evidence: route }, guidance: { currentPhase: null, nextPhase: "orient", nextPurpose: "orient", requiredEvidence: [], phaseSequence: ["orient"], atFinalPhase: false, disposition: "read-compute-route", effects: ["read", "compute"], requirements: [], blockers: [], nextAuthority: "EVAVO-STUDIO/test", handoffRecommended: false } };
  verifyAgentWorkbenchGuidance({ value: guidance }, { value: snapshot, bytes: snapshotBytes, sha256: snapshotSha256 });
  const mcpGuidance = { ...base, selectedRoute: { type: "tool", id: "mcp:test", selection: "explicit", relevance: 1, sha256: digest(Buffer.from(JSON.stringify(mcpRoute), "utf8")), evidence: mcpRoute }, guidance: { currentPhase: null, nextPhase: "orient", nextPurpose: "orient", requiredEvidence: [], phaseSequence: ["orient"], atFinalPhase: false, disposition: "mcp-runtime-verification-required", effects: [], requirements: [], blockers: ["Obtain a live protocol/discovery receipt for the exact server before any tool call or effect claim."], nextAuthority: "EVAVO-STUDIO/test", handoffRecommended: false } };
  verifyAgentWorkbenchGuidance({ value: mcpGuidance }, { value: snapshot, bytes: snapshotBytes, sha256: snapshotSha256 });
  const unsafe = structuredClone(mcpGuidance);
  unsafe.selectedRoute.evidence.runtimeReadiness = "ready";
  let rejected = false;
  try { verifyAgentWorkbenchGuidance({ value: unsafe }, null); } catch { rejected = true; }
  assert(rejected, "Guidance verifier must reject ready MCP launch evidence.");
  process.stdout.write(`${JSON.stringify({ contract: "evavo_agent_workbench_guidance_verifier_self_test_v2", status: "passed", assertions: 3 }, null, 2)}\n`);
}

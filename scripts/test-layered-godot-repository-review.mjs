...
test("reviews an exact seven-file untracked handoff without staging or mutation", async (t) => {
  ...
  assert.equal(result.admission.immutableInputSnapshot, true);
  assert.equal(result.admission.exactHandoffReceiptContract, true);
  assert.equal(result.admission.unsupportedInputFieldsRejected, true);
  assert.equal(result.admission.dependenciesCapturedBeforeAsyncBoundary, true);

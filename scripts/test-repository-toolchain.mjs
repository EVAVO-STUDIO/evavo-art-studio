...
try {
  reset();
  assert.equal(run().status, 0, "exact fixture must pass");
  ...
  assert.notEqual(run().status, 0, "an unreviewed lockfile must fail");
  ...
  console.log("- Node.js, pnpm, lockfile, workflow and capability drift fail closed");
} finally {
